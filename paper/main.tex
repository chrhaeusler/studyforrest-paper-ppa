...
\todo[inline]{reviewer #1: its clear that where (spatial processing) must take
place in both streams. You should probably acknowledge that.}
% PPA
...
% block-design
2) used a block-design paradigm to maximize detection power, and
% task
3) they usually employed an explicit (perceptual) judgement task.
...
\centering
    ...
    \caption{Fixed-effects individual-level GLM results ($Z$>3.4, $p$<.05
        ...
        (\texttt{vse\_new > vpe\_old}).
        Black:
        outline of participant-specific PPA ROIs reported by
...
aspects inherent in pictures or movie shots of landscapes.
% aPPA
...
inherent in both visual and verbal stimuli.
...
Scripts to generate the results as Datalad \citep{datalad} are published to a
GIN repository \href{https://gin.g-node.org/chaeusler/studyforrest-ppa-analysis}{\url{gin.g-node.org/chaeusler/studyforrest-ppa-analysis}} (DOI assignment pending).


\section*{Data Citations}

\todo[inline]{Following dataset are added to references.bib}
\todo[inline]{reference to cut anno might be retundant}
\todo[inline]{propably, cite them above in the manuscript where it makes sense}
...
based on events of the annotated movie cuts (Table~\ref{tab:contrasts}) in order
...
\todo[inline]{Just mention this part in the main document via 'the corresponding
surface plots (Fig xy) can be found in the supplemental materials'?}
\todo[inline]{write intro regarding purpose of additional surface plots?}

% reconstruction MNI
Cortical reconstruction of the MNI152 T1-weighted brain template
[MNI152\_T1\_1mmi\_brain.nii.gz] was performed using
\href{https://surfer.nmr.mgh.harvard.edu}{FreeSurfer} v7.1.1
\citep{dale1999cortical}.
% reconstruction: individual results
Subject-specific cortical surfaces are part of pre-existing
...
% smoothing
Group and individual PPAs were additionally patially smoothed applying a
Gaussian kernel with full width at half maximum (FWHM) of \unit[3.0]{mm}.

\subsubsection*{Group results}
Group results see Table~\ref{tab:res-ao-group1} for AO results,
Table~\ref{tab:res-av-group1} for AV results, and
Figure~\ref{fig:grp_ao_av_c1_surf} for surface maps.


\begin{figure*}[tbp]
\centering
    \includegraphics[width=\linewidth]{figures/grp_ao_av_c1_surf}
...
Group results see Figure~\ref{fig:grp_ao_av_stability} for surface maps.


\begin{figure*}[tbp]
\centering
    \includegraphics[width=\linewidth]{figures/grp_ao_av_stability_surf}
    \caption{Overlap of significant clusters ($Z$>3.4; $p$<.05, cluster
        corrected) across all contrasts for both naturalistic stimuli.
        The audio-description's contrasts 1-8 (blue)
        are overlaid over the audio-visual movie's contrasts 1-5 (red;
        \ref{tab:contrasts}).
        Cluster are projected onto the surface of the
...
\subsubsection*{Individual resultsStability}

Individual results see Figure~\ref{fig:subjs_ao_av_c1_surf} for surface maps.
\todo[inline]{RSC in of AV is essentially not visible}
...
\centering
    ...
    \caption{Fixed-effects individual-level GLM results ($Z$>3.4, $p$<.05
        cluster-corrected)
        projected onto subject-specific brain surface that were
        [re]constructed from individual T1-weighted brain images.
        The results of the audio-description's primary
        $t$-contrast (blue) that compares geometry related nouns to non-
        geometry related nouns spoken by the narrator
        (\texttt{geo, groom > all non-geo}) are overlaid over the movie's
