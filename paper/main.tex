...
static pictures of landscapes, buildings or landmarks \citep{aguirre1998area, epstein2014neural, epstein1998ppa} compared to e.g. pictures of tools of faces.
...
from Huth (2016); good voxel-wise semantic modeling regarding places in PPA but
imo that does not say anything about excitation/inhibition; interestingly Huth
mentions PPA during talks but it is not discussed in any paper and despite being
promised none of his group came up with a paper regarding that topic}
...
echo-planar images (gradient-echo, \unit[2]{s} repetition time,
...
detection power due to shared variance among regressors.\todo{sounds meh but
don't know how do write it better}
...
soundscape that already cue a change of settings.\todo{cf. contrast that uses
...
the soundscape.\todo{cf. contrast that uses cuts but in AO stimulus}
...
\todo[inline]{s. comments for overview of individually significant clusters}
...
regions of the group PPA overlap.\todo[inline]{temporal occipital fusiform gyrus
according to Harvard-Oxford Cortical Structural Atlas}
% Precuneus = retrosplenial cortex ?
Six of eight contrasts show significant clusters in the retrosplenial region
...
\todo[inline]{provides caveats to the interpretation describes how paper advances the field by providing new opportunities}
...
\todo[inline]{e.g. We demonstrate\dots; we discovered\dots; our data provide
evidence\dots}
...
Hence, we tested if the perception of verbal spatial cues embedded in an
auditory narrative contrasted with non-spatial cues correlate with increased
activation in the PPA.
% ``how'' we did it
We use analogous statistical models in three different experimental paradigms to
compare results gained from a dedicated visual localizer to results we gained
from a movie and the movie's audio-description.
...
interpretation is already given there already}
...
% intro
We performed whole-brain analyses on a group level and individual level. In both
naturalistic stimuli, significantly increased activity in the PPA correlates
with spatial events compared to non-spatial events.
...
% no task in Epstein
An early block-design study \citep{epstein1998ppa} that compared results from a
paradigm that employed a (perceptual) judgment task of static pictures to a
paradigm without that task.
...
features were incidentally embedded in a continuous auditory (or audio-visual)
stream.
% conclusion
...
Our primary contrasts compared verbal cues regarding landmarks ((\texttt{geo};
often buildings or aspects of a whole scene) and elements defining the geometry
of a room (\texttt{groom}) to non-spatial cues.
% vs vision
In the visual domain, whole landscapes and not pictures of landmarks are
considered to be the ``optimal'' stimulus type (cf. Epstein (1998, 1999, 2003,
2008).
% our se_new & se_old
The audio-description's narrator rarely described a whole landscape by a single
word (e.g. ``in a football stadium'', ``Forrest is running through the jungle'')
when the plot switch from one setting to another setting but usually by
continuously adding more concrete spatial cues (e.g. landmarks like buildings,
trees, and streets)
% further studies
Humans can identify the gist of a rich visual scene within the duration of a
single fixation \citep{henderson2003human}. Hence, further studies might
investigate if vague verbal descriptions of a scene as a whole lead to
differential hemodynamic activity in the PPA compared to more concrete spatial
cues.
...
% it shows two thing
\todo[inline]{in how many clusters are how many overlapping clusters}
% no incidental findings
The results of the additional contrasts suggest that increased activation within
the PPA is not just an accidental finding.  The bilaterally increased activation
in the PPA does not depend on accidental findings but are stable across
contrasts.
% decline in isolation power
Nevertheless, the ability to isolate the PPA drops the less the amount of data
are available and the less the events averaged to built a category match the
preferred stimulus type processed in the PPA.
% decline b)
...
events within categories matched on average the preferred stimulus type. (cf.
\texttt{se\_new} and \text{se\_old} in contrast 6-8)
% concluding statement hypothesis
Moreover, nouns of the narrator cueing a switch to another setting are often
forestalled by changes of the soundscape.
% arnott
\todo[inline]{arnott2008crinkling find increased activations of PPA to sound
made by objects}
% hypothesis
...
\todo[inline]{arnott2008 might be a good opportunity to switch to more general
assumption of ``contextual information'' and combine that switch with
anterior/posterior discussion}


\paragraph{bilateral superior lateral occipital cortex}
...
contrast in the retrosplenial cortex (RSC).\todo{how many exactly?}
...
\todo[inline]{AV stimulus: we DID NOT find clusters that could be attributed to
