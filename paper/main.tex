...
\item revise captions of Table 1 + 3; regressors that are based on the other
    stimulus for control purposes are now dropped from the table; but mention in
        Statistical Analysis / Statistical Model)

\item what do we need to check to ensure subsequent analyses are valid;
    a) correlation of regressors (first "sanity check" but more a prerequisite)
    b) negative controls (actual sanity check)
...
\begin{itemize}
\item mehrere Kontraste in VIS genommen, hier einer berichtet: Teil aus
    Discussion in Results hochnehmen
\end{itemize}

\noindent Discussion
\begin{itemize}
    \item \citep{bartels2004mapping, hasson2004intersubject}
    \item VIS: "best contrast was chosen"
\end{itemize}
...
% hypo b
Focussing on the [``cleaner operationalization'' of the] audio-description, we
further hypothesize[d] that an exclusively auditory stimulus can/could
substitute a block-design experiment using pictures to localize a ``visual
area'' in individual subjects.\todo{needs short statement on why one would even want that}
% results: group
...
% intro
Lastly, we algorithmically annotated the temporal
structure of low-level perceptual features to create nuisance regressors.
% visual shit
...
the perceptual differene of consecutive frames.
% auditory shit
Finally, we computed the left-right difference in volume and root mean
square energy averaged across the length of every movie frame (\unit[40]{ms}) of
both audio-tracks.
...
modeled as impulse events [lenght = 20 ms?].\todo{actually, that might not be
true}
...
\todo[inline]{SE: gab es ein separates null-event für jeden zeitraum zwischen
zwei annotierten events oder war das ein regressor der immer wieder kam?}
...
% convolving
Events for all regressors of the AV and AO timulus were convolved with a
...
% fixed time course; detection power
Since we annotated events in fixed time courses that could not experimentally
manipulated, regressors that are based on these events could be correlated
leading to shared variance and thus low detection power of subsequent analyses.
% correlations within stimuli and across stimuli
Hence, we first tested for correlation of regressors across the eight segments
of each stimulus and correlations of regressors across both stimuli (s. Figure
\ref{fig:reg-corr})
% correlations within each stimulus
The computed Pearson correlation coefficients demonstrate no to minor
correlations of regressors of the same stimulus [which suggest the models should
not suffer from low detection power due to shared variance among regressors].
% correlations across stimuli
The highest correlations are apparent between auditory low-level confounds
across stimuli. This was expected and served as ``sanity check'' since the
audio-track of the audio-description is the audio-track of the movie but
featuring the additional narrator (s. Figure \ref{fig:reg-corr}).
...
\todo[inline]{regressors of other stimulus in GLM of other stimulus}
% categories taken from movie cut annotation
Maybe only say in table caption: To create contrasts of cross-modal negative
control and to capture variance of a possibly confounding change of the
soundscape after a cut, we also implemented the five movie cut-related
regressors used in the GLM of the AV stimulus (see details below) in the model
of the AD stimulus.

% contrasts
Based on these regressors, we then created $t$-contrasts that we presumed to
subtract variance correlating with the perception of non-spatial information
from variance correlating with the perception of spatial information.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% "ingredients" to answer our questions (cf. intro); we have annotations; define
% congruent contrasts based on regressors based on annotations

% repeat key question (cf. end of intro)
The key question of the current study was if a) we could operationalize the
perceptual processes in an audio-visual and exclusively auditory
naturalistic stimulus and if b) contrasting that fcompared to an
operationalization of non-spatial perception  would yield to increased
hemodynamic activity in the PPA as defined by a visual localizer experiment.
% difference (kinda repetition)
Instead of carefully selecting and designing conceptualized/simplified stimuli
that we assumed to match the proposed perceptual processes, we selected and
categorized stimulus features (movie cuts and spoken nouns) that are embedded in
two naturalistic stimuli.

\paragraph{included regressors}
...
% intro
To test the robustness of our approach, we operationalized the
perception of spatial information compared to the perception of ``non-spatial''
information differently in eight $t$-contrasts for the AD stimulus (see Table
...
event counts).\todo{make ref to what is no "Audio-description: annotation"}
% thus: primary contrasts
For each stimulus, we chose one contrast to be ``primary contrast'' for further
inspection and a detailed report.
...
\todo[inline]{one could write more about the different contrast in Segupta et
al. here, but imo it's sufficient to point that out in the results given that
all subjects except two used the strict set given contrasts of VIS study and
that the ``best working'' will be used}

\todo[inline]{Nevertheless, state it in one sentence and check Berman, M. G. et al. Evaluating functional localizers: The case of the FFA. NeuroImage 50, 56–71 (2010)}
...
\subsubsection{model testing / GLM computations}

To test our model, we ran several $t$-contrasts as part of a GLM for each
stimulus.
...
\section{Results}
\todo[inline]{we have to decide how we want to present this: a) by stimulus or b) by analysis type. With the present abstract I am leaning towards (a), but we should debate that}
...
\paragraph{AV: other significant clusters across contrasts}
