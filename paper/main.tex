...
\todo{print contrasts; count per category? -> pattern "average of events" and
counts vs. significant clusters?}

\subsubsection{PPA, RSC, and LOC (AV \& AV)}

\todo[inline]{SE: use different colormap for overlap of contrasts' clusters}
lche Kontraste (z.B. im Ergebnis) von anderen abgewichen?}
...
%
\todo[inline]{print contrasts; counts per contrasted categories? "fit of
averaged events"?}
% but
...
\todo[inline]{following list probaly exhaustive but probably too long}
%
...
% AV using nouns
The contrast that used the narrator's nouns that did not actually exist in the
movie stimulus (\texttt{se\_new} > \texttt{se\_old}) yielded a significant
cluster in the right PPA and bilateral clusters in the superior lateral
occipital cortices (the right hemispheric cluster extending into the superior
parietal cortex).\todo{explanation?}

% AD stimulus
For the analysis of the audio-description, we created control contrasts based on
events of the annotated movie cuts (Tab.~\ref{tab:ao-contrasts}), which although
invisible were nevertheless frequently perceivable due to changes in the
soundscape.
% contrast 9
...
% AV anno in AV data
The the 5 movie PPA contrasts but with the movie data using cuts. All null
results except:
% contrast 9
