...
and their spatial layout at the beginning of a movie.
...
\tabularnewline
\texttt{geo-room} & rooms \& locales or geometry-defining elements & living room; wall, door, window, floor
\tabularnewline
...
Given that all planned voxel-wise general linear models (GLM) exclusively involve
regressors from one of the naturalistic stimuli, the observed correlation
pattern does not indicate problematic confounds of nuissance variables and
...
participant-specific results across the 8 segments for every participant and each
stimulus.
% level 3
Subsequent, third-level analyses averaged contrast estimates over participants.
...
differently in five $t$-contrasts for the movie stimulus (Tab.~\ref{tab:av-contrasts})
and eight $t$-contrasts for the audio-description stimulus (Tab.~\ref{tab:ao-contrasts}).
% reason: robustness
...
% AD stimulus
For the audio-description stimulus, we created five $t$-contrasts
(Tab.~\ref{tab:ao-contrasts}) to test if increased activation in the PPA was
...
participant and segment was performed in functional space preserving the orientation
of the EPI images.
% two ways of co-registration
Participant-specific BOLD T2* time series were anatomically co-registered in two
ways:
...
a) via linear transformation to a participant-specific, study-specific T2* template
...
precuneus \\ % 0.0000000596
164 & <.00001 & 4.47 & 17.5 & -58 & 23 & 15.6 & -55.6 & 16 & r. precuneus;
\\ % 0.000000238
83 & .00013 & 4.48 & 27.5 & -43 & -17 & 27.2 & -41.1 & -14 & r. temporal
(occipital) fusiform c.; posterior parahippocampal g. \\ % 0.000128
73 & .00031 & 3.93 & -22.5 & -43 & -12 & -23.9 & -43.6 & -11.2 & l. lingual
g.; temporal (occipital) fusiform g., posterior parahippocampal c. \\ % 0.000128
...
individual participants to ensure that the group results are not just a by-product
of the averaging process.
% AD is the real shit
Further, we hypothesized that an exclusively auditory stimulus could localize
the PPA in individual participants and thus possibly substitute a classic visual
...
participants and an unilateral right clusters in two participants (Tab.~3 in
\citep{sengupta2016extension}).
% Sengupta: 1 of 3
...
isolated cluster (Tab.~3 in that matched the expected anatomical location
...
each naturalistic stimulus and the same threshold for all participants.
...
participant (Fig.~\ref{fig:bland-altman}).
% what it depicts
...
\todo{clusters are pretty big too}
%
