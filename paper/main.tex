...
\end{abstract}
\todo[inline]{Vorgabe: Intro + Discussion = main text ~ 3,000 words; einige offene Dinge sind folgend in Kommentaren und nicht als ToDo gemarkt}
...
\todo[inline]{Egal, was ich folgend in der Überblick \& später im Detail bzgl. Aziz (2008) schreibe, es wird wahrscheinlich nicht korrekt sein; so wie ich es verstanden habe, hat Aziz den Unterschied zwischen PPA und FFA angeguckt, aber diesen Unterschied im Kontrast zu object-related Sentences; s. dortige Fig 3; im Grunde kommt da aber überall nur uneinheitliche Grütze raus => bitte gemeinsam im Gespräch checken}
...
% Aziz hat auf eine zweite Mail geantwortet: Sie modelt die HRF über den jeweils gesamten Satz von ca. 2.8 Sekunden.
...
\todo[inline]{\dots and compared those to a control condition of object-related sentences}
...
\todo[inline]{aber Huth, 2016, implizit; diskutiert PPA als solches jedoch gar nicht -> noch mal checken}
...
\todo[inline]{statement how often datasets have been analyzed be independent groups -> Frage: ist die Liste auf psychoinformatics.de ist aktuell/erschöpfend? Ist dann schnell eingebaut}
...
\todo[inline]{um Sengupta ergänzen}


\subsection{Participants}
\todo[inline]{ich habe von den aligned data eine Intersection von 14 VPN aus AO und AV; in der Speech-Anno-Validation (7T only) habe ich 19 VPN; es fehlt jeweils VP10; demographische Daten bzgl. (Durchschnitts)Alter und Geschlecht ist folglich falsch}
...
\todo[inline]{Daten zu Sengupta einarbeiten}
...
\todo[inline]{Warum wurden diese Kategorien so gewählt}
\todo[inline]{Maybe just cite the table}
...
% data source
\todo[inline]{wie immer bzgl. Motion Correction und Registratziohier ggf. expliziter erwähnen, dass AO spatially downsampled ist}
...
\todo[inline]{cross reference to fig:reg-corr}
...
\todo[inline]{wenn ich folgend den "null regressor" nehme, darf ich den ganzen Run, glaube ich, in der 2nd level Analyse gar nicht benutzen??}
For the AO stimulus, we used the narrator's ``descriptive nouns'' (see Table \ref{tab:descr-nouns}.

\todo[inline]{some categories were pooled, because\dots}
...
\todo[inline]{make clear which stimulus each annotation is based on, and make clear(er) what defined an event. This could be a big table, because good defintions are important for judging the results}
\begin{table*}[t]
    \caption{Overview of events to build the 18 regressors for the analysis of the audio-only (AO) stimulus. Some of the original categories (female, females, fname; male, males, mname; face, head; see Table \ref{tab:descr-nouns}) were pooled into new categories (sex\_f; sex\_m; fahead)
...
\midrule
body & descr & \aoBodyAll & \aoBodyI & \aoBodyII & \aoBodyIII & \aoBodyIV & \aoBodyV & \aoBodyVI & \aoBodyVII & \aoBodyVIII \tabularnewline
bpart & descr & \aoBpartAll & \aoBpartI & \aoBpartII & \aoBpartIII & \aoBpartIV & \aoBpartV & \aoBpartVI & \aoBpartVII & \aoBpartVIII \tabularnewline
fahead & descr & \aoFaheadAll & \aoFaheadI & \aoFaheadII & \aoFaheadIII & \aoFaheadIV & \aoFaheadV & \aoFaheadVI & \aoFaheadVII & \aoFaheadVIII \tabularnewline
furn & descr & \aoFurnAll & \aoFurnI & \aoFurnII & \aoFurnIII & \aoFurnIV & \aoFurnV & \aoFurnVI & \aoFurnVII & \aoFurnVIII \tabularnewline
geo & descr & \aoGeoAll & \aoGeoI & \aoGeoII & \aoGeoIII & \aoGeoIV & \aoGeoV & \aoGeoVI & \aoGeoVII & \aoGeoVIII \tabularnewline
groom & descr & \aoGroomAll & \aoGroomI & \aoGroomII & \aoGroomIII & \aoGroomIV & \aoGroomV & \aoGroomVI & \aoGroomVII & \aoGroomVIII \tabularnewline
object & descr & \aoObjAll & \aoObjI & \aoObjII & \aoObjIII & \aoObjIV & \aoObjV & \aoObjVI & \aoObjVII & \aoObjVIII \tabularnewline
se\_new & descr & \aoSenewAll & \aoSenewI & \aoSenewII & \aoSenewIII & \aoSenewIV & \aoSenewV & \aoSenewVI & \aoSenewVII & \aoSenewVIII \tabularnewline
se\_old & descr & \aoSeoldAll & \aoSeoldI & \aoSeoldII & \aoSeoldIII & \aoSeoldIV & \aoSeoldV & \aoSeoldVI & \aoSeoldVII & \aoSeoldVIII \tabularnewline
sex\_f & descr & \aoSexfAll & \aoSexfI & \aoSexfII & \aoSexfIII & \aoSexfIV & \aoSexfV & \aoSexfVI & \aoSexfVII & \aoSexfVIII \tabularnewline
sex\_m & descr & \aoSexmAll & \aoSexmI & \aoSexmII & \aoSexmIII & \aoSexmIV & \aoSexmV & \aoSexmVI & \aoSexmVII & \aoSexmVIII \tabularnewline
sex\_u & descr & \aoSexuAll & \aoSexuI & \aoSexuII & \aoSexuIII & \aoSexuIV & \aoSexuV & \aoSexuVI & \aoSexuVII & \aoSexuVIII \tabularnewline
% vlo_ch has no events in segment 5. So indices are shifted
vlo\_ch & descr & \aoVlochAll & \aoVlochI & \aoVlochII & \aoVlochIII & \aoVlochIV & 0 & \aoVlochV & \aoVlochVI & \aoVlochVII \tabularnewline
vno\_cut & descr & \aoVnocutAll & \aoVnocutI & \aoVnocutII & \aoVnocutIII & \aoVnocutIV & \aoVnocutV & \aoVnocutVI & \aoVnocutVII & \aoVnocutVIII \tabularnewline
vpe\_new & descr & \aoVpenewAll & \aoVpenewI & \aoVpenewII & \aoVpenewIII & \aoVpenewIV & \aoVpenewV & \aoVpenewVI & \aoVpenewVII & \aoVpenewVIII \tabularnewline
% vpe_old has no events in 3. So the indices are shifted
vpe\_old & descr & \aoVpeoldAll & \aoVpeoldI & \aoVpeoldII & 0 & \aoVpeoldIII & \aoVpeoldIV & \aoVpeoldV & \aoVpeoldVI & \aoVpeoldVII \tabularnewline
vse\_new & descr & \aoVsenewAll & \aoVsenewI & \aoVsenewII & \aoVsenewIII & \aoVsenewIV & \aoVsenewV & \aoVsenewVI & \aoVsenewVII & \aoVsenewVIII \tabularnewline
vse\_old & descr & \aoVseoldAll & \aoVseoldI & \aoVseoldII & \aoVseoldIII & \aoVseoldIV & \aoVseoldV & \aoVseoldVI & \aoVseoldVII & \aoVseoldVIII \tabularnewline
...
fg\_ad\_rms & root mean square & \aoFgadrmsAll & \aoFgadrmsI & \aoFgadrmsII & \aoFgadrmsIII & \aoFgadrmsIV & \aoFgadrmsV & \aoFgadrmsVI & \aoFgadrmsVII & \aoFgadrmsVIII \tabularnewline
\bottomrule
\end{tabular}
\end{table*}
