...
% conceptualized stimuli and blockl: disadvantages
...
reduce complexity and explain these better}

\todo[inline]{Simon suggested to put paragraphs regarding annotation here,
    followed by a subsection ``Regressors''. Additionally imo, it makes sense to
    build one subsection ``Stimulus annotations \& regressors'' to have a
    structure of ``AO (anno \& regressors); AV (anno \& regressors'' instead of
AO \& AV (annotation), AO \& AV (regressors)``.}
...
operationalized the perception of landscapes and landmarks (compared to e.g.
faces and objects) by annotating words spoken by the narrator of the
audio-description as well as cuts and depicted locations in the movie.
...
% table for descriptive nouns: categories, rules, examples counts
\begin{table*}[h!]
    \caption{Descriptive nouns spoken by the audio-description's narrator that
        served as a basis to build the regressors
(see Table \ref{tab:ao-events}):
    categories, annotation rules, and examples (given in English).
