...
from on- to offset of single words (that indicates the visual content to the
listener).
% conclusion
...
faces.\todo{average response across voxels in ROI}
%
...
landscapes (e.g.  ``[in a] football stadium'', ``[Forrest is running through
the] jungle'') but mostly landmarks or buildings and also non-spatial hints
(e.g. ``[Jenny as a] teenager``).
% further studies
...
of localizer}
%
Based on the fact that the annotation of nouns still offered a more precise way
to operationalize the perception of spatial information than the annotation of
cuts and locations, we also hypothesized that analyzing the data from the
...
The audio-description's primary contrast that we used for all subjects yielded
significant bilateral clusters in 9 of 14 subjects (of which sub-04 shows only a
right-lateralized PPA ROI), and a unilateral significant cluster in one subject.
...
% PPA AV
Regarding contrasts of the AV stimulus (see Table \label{tab:av-contrasts}), all
five AV contrasts show significant bilateral clusters in the PPA but also
extending into more posterior brain regions.
% PPA AD
Regarding contrasts of the AD stimulus (see Table \label{tab:ao-contrasts}), six
...
clusters spatially constrained to the anterior part of the PPA group overlap as
defined by the visual localizer [as well as constrained to the anterior part of
significant cluster of AV contrasts].
% question
...
\todo[inline]{part regarding RSC \& LOC is now super short; maybe give half a
sentence that this was not focus of the story (but still kinda expected)}
...
stimulus: 7 of 8 contrasts) and LOC (AV stimulus: XY of 5 contrasts; AD
stimulus: XY of 8 contrasts) are known to be scene-selective regions (see
\citep{chrastil2018heterogeneity, bettencourt2013role} for reviews).
% conclusion
\todo[inline]{conclusion: what does that say about our results regarding PPA as
a scene-selective region, and audio-visual and auditory naturalistic stimuli in
general?}

\todo[inline]{if we locate a conclusion regarding ``naturalistic stimuli, fuck
yeah'' (+ re-use of existent data) here already, we can add the following part
``not many random clusters'' \& ``control contrasts'' imo elegantly and
shortened here as ``caveats''}

\todo[inline]{``caveats'' = you need extensive annotations and need to know the
    confound structure; but it's the confound structure that leads to the
    hypothesis that changes of the soundscape might also correlate with activity
in the PPA (cf. ``shower thoughts'')}
...
\todo[inline]{make this short; as expected almost everywhere null results ->
natural stimuli = suitable}

\todo[inline]{similarly: we have a pattern of spatially restricted clusters
across contrasts (and stimuli) in the PPA contrasts -> natural stimuli =
suitable}
...
\subsubsection{shower thought: changes in soundscape should also correlate with
PPA activity}
