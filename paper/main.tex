...
\begin{itemize}
    ...
    \item there is no "cue about" in the English language? alternatives are
        "give a hint on/about", "indicate"; changes the sentences but should
        ask Alex
\end{itemize}
...
correlated; and now what? -> cf. discussion, part "random shower thoughts"}
% control contrasts
...
\textit{relaxed} set, or finally the \textit{simple} set\todo{not used for PPA},
and repeated the process until we found a cluster that matched the expected
...
For better comparisons of subjects, we report results from the analyses
...
% Bland-Altman-Plots
To better depict the agreement and difference between individual results of the
visual localizer and the primary AD contrast, we created Bland-Altman-Plot that
visualize mean and difference of two spatially corresponding $z$-values (see
Figure \ref{fig:bland-altman}).
% top KDE plot
...
\todo[inline]{I tried to keep it concise but detailed; individual contrasts
are given but as numbers}
...
% non-systematic
Lastly, contrast 2 of the AD stimulus  also shows bilateral clusters in the
anterior cingulate gyrus and right paracingulate gyrus (frontal medial cortex),
right putamen, and right frontal pole.
%
Contrast 3 shows clusters bilaterally in the anterior cingulate gyrus and left
frontal pole.
%
Contrast 6 shows one additional cluster also in the right medial frontal
cortex.\todo{fsleyes (again) to match text such that clusters in Figure 5 are
easily attributable to region}
...
perception via general linear model (GLM) $t$-contrasts.
% current study: dissimilar
Contrary to previous studies, our current operationalization is not based on
...
\todo[inline]{AV is "intermediate step": natural but still visual; similar:
\citep{bartels2004mapping, hasson2004intersubject} (mention here already or at
the end of topic)}
...
low amount of random clusters in the brain)}
...
cut and did not control for confounding features like depicted faces or objects
or attentional focus of subjects.
% shortcomings
m.a.W.: Results of the AV stimulus that we ran as a conceptual control are
tempered by using different kind of movie cuts regardless of the visual content
of the movie frame after the respective cuts.
% which leads to the ``blurred'' results

\todo[inline]{we exploit a confound induced by cinematography; field size
decreases later in the movie; this is a confound based on cinematography => the
stimulus changes over time; (ergo: man sollte sich bißchen mit Cinematographie
auskennen und/oder stimulus annotation in model nehmen, wenn man was mit movies
machen will)}
...
\todo[inline]{Nevertheless, group results from analysis using more natural
visual stimulation suggest that... (cf. \citep{bartels2004mapping,
hasson2004intersubject})}

\todo[inline]{approach of contrasting cuts was adventorous, annotation of speech
suited better in the first place, and the auditory thing ist more interesting
anyway, we did not aim for a substition of classic localizer via movie; but if
you wanna do it, you should...:}
...
\todo[inline]{since the picture of PPA is far more unclear in auditory
domain and we have a pretty nice stimulus with a narrator and a
pretty nice annotation, we focussed on the audio-description}
