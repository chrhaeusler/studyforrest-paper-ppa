...
\todo[inline]{possibly drop some words in ``how''-``what'' sentence if it's
considered to be wordy}
...
% open question
\todo[inline]{I revised the following paragraph; I guess it's okay that way} In
this study, we investigate whether increased hemodynamic activity in the PPA
...
\citep{hanke2016aligned}.\todo{new ref. to dataset}
...
functional localizer performed with the same set of participants
\citep{sengupta2016extension, sengupta2016extensiondata}.\todo{new ref. to
dataset}
% intro to hypo a
...
hemodynamic activity in medial temporal regions that were functionally defined
as the PPA by the analysis of the localizer data.
%hypo b: individuals
...
\todo[inline]{I tried to keep the summary of methods short and use a pretty
similar phrasing; still, I wouldn't get the result section and read the Method
section and tables there while reading the results\dots. Most important imo is
to link the Tables (Events + Contrasts) and explain that there is that ``primary
contrast'' thing.}
...
with events that should not (or to a less degree) correlate with non-spatial
perception (see Table~\ref{tab:events}).
% from supplementals
...
For each stimulus, we chose a primary contrast for result presentation (marked
with an asterisk in the table) based on a subjectively assessed balance of how
...
% group level
On a group average level, we here report results from whole-brain analyses of
...
\todo[inline]{text is change to reference volume and surface plots; the
``method'' of creating the surface plot is written here because it is just one
sentence}
...
\citet{baldassano2013differential} propose that the PPA creates a complete
...
that were averaged across voxels of regions of interest (PPA and fusiform face area, FFA
...
% method
We took advantage of three\todo{actually two} fMRI datasets and two stimulus annotations that are
...
\citep{hanke2016aligned}\todo{new ref.} that were used for the technical validation analysis
...
% raw data
The realigned participant-specific timeseries that were used in the current analyses were derived from the raw data releases and are available as Datalad
...
\href{https://gin.g-node.org/studyforrest/cortical-surfaces-freesurfer}{\url{gin.g-node.org/studyforrest/cortical-surfaces-freesurfer}}
\citep{hanke2016freesurferdata}), and are also part of the original data release
on \emph{OpenfMRI} (\url{https://legacy.openfmri.org/dataset/ds000113}\citep{Hanke2014ds000113}.
