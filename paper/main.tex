...
\todo[inline]{Limit for Intro + Discussion (main text) = ca. 3k words;
intro is about 1k words (with to dos)}


\todo[inline]{RSC is not mentioned in intro anymore but it shows up in group
results (mixed in individual subjects) and therefore could be discussed}
...
regions of the fusiform gyrus \cite{epstein1998ppa} and anterior lingual gyrus
...
Abschnitte sind in der PDF markiert. Ihre Method Section ist ``strange'' und im
Grunde kommt überall nur uneinheitliche Grütze heraus; ich blicke nicht durch,
weshalb folgendes nicht vollständig korrekt sein wird;so wie ich es verstanden
habe, hat sie den Unterschied zwischen PPA und FFA angeguckt, aber diesen
Unterschied im Kontrast zu object-related Sentences (dortige Fig 3)}
% Aziz' stimuli
To our knowledge only one study \citep{aziz2008modulation} correlated
...
\todo[inline]{I deleted sentences regarding competing ``contextual associations
    hypothesis'' (\citep{aminoff2006parahippocampal, aminoff2013role}) because
our events/regressors cannot touch that issue; might be interesting during
...
imaging methods in individual diagnostics (cf. \citep{dubois2016building,
eickhoff2020towards}).
% open questions
...
audio-only stimulation\todo{``audio-only'' sounds meh; ``exclusively auditory
stimulation''?}.
% Huth (2016)
\todo[inline]{Huth et al. (2016) implicitly regarding audio-only; but they do
not discuss the PPA; in general, they do voxel-wise encoding which doesn't say
anything about excitation/inhibition}
...
continuous stimulation that better resemble our experience of a our dynamic
environment.
...
address the question how the investigated functional areas respond an audio-only
stimulus.
...
an audio-only paradigm and test its localization performance of a ``visual
area'' in individual study participants.
% comparison to dedicated localizer
Finally, we compare these results to results of a dedicated visual localizer
experiment done with the same participants \citep{sengupta2016extension}.

\todo[inline]{maybe insert ``foreshadowing'' of results here?}
...
\todo[inline]{explain as introduction to every step why it woes done}
\todo[inline]{current order of subsections might be ``non-canonical''}
\todo[inline]{parts from original studies might still be too detailed}
...
\todo[inline]{put stuff regarding hierarchical categories and heterogeneity
here; counts in table show the counts of just the primary category; settings
changes often (30-60 \%) belong to another category; most often it's geo but
also object or male/mname/furniture; cf. Results and Discussion regarding
...
of both naturalistic stimuli. Results demonstrate no to minor correlations
(s. \ref{fig:reg-corr}) which suggest the models should not suffer from low
...
\todo[inline]{contrasts are ordered by ``how well they make sense'' according to
    theory, and amount of events/data; Hence, they were assumed to result in
    more and more shitty results (s. introductory sentence in results); at the
    moment all 8 contrasts are used in the result (figure) despite the low
ranked COPEs yielded 0 result as kinda expected}
...
of the contrasts of interest because nouns in this category could either be
considered as object or as a geometry-defining structure of a room.
\todo[inline]{but: Epstein (2008): response to arrays of objects drawn from the
rooms but displayed on a blank background is quite weak [11].}
...
the soundscape (anticipating the narrator's cue of a switch to new setting,
too).
...
audio-description to create conventional GLM contrasts of of modeled
hemodynamic activity resembling analyses of previous studies using simplified
stimuli.
% comparison AO & AV
...
\todo[inline]{paragraphs start with a sentence or two that set up the question
that the paragraph answers; e.g. ``To verify that there are no artifacts; What
is the test-retest reliability of our measure?''}
\todo[inline]{middle of the paragraph presents data and logic that pertain the question}
\todo[inline]{paragraph ends with a sentence that answers the question via logical statement; for example, it may conclude that none of potential     artifacts were detected. Each paragraph convinces the reader of answer given
in its last sentence}


\todo[inline]{at the moment I think it is better to report the primary contrast
fist and then report the stability of contrast; but that opinion changed twice
already.}
...
\todo[inline]{other reasons than localization performance? look which subjects influence the group results the most? ``multiple localizer''?}
...
Individual ROIs were assessed by \citep{sengupta2016extension} by visually
inspecting unthresholded $z$-maps, and setting a subjectively best fitting
...
We find bilateral clusters in sub-20, in which the dedicated localizer yielded
...
the visual localizer and contrast 1 of the AO paradigm, we created one
...
\todo[inline]{at the moment (which changes regularly) this section should follow
the other group results; depends on the ``final'' discussion.}

\todo[inline]{I consider tables of significant clusters for every contrast as
overkill. One contrast, the ``primary contrast'', is appropriate; s. comments
for overview of individuals}
...
We created a variety of contrasts to test a) if our approach yielded
similar results across similar contrasts and b) if localization performance
decreased when the contrast included categories that contained stimuli less
congruent with the ``optimal'' stimulus type\todo{and the less data we use}.
...
contrast depends on (just) using the categories \texttt{se\_new} and
\texttt{se\_old} which were assumed to be problematic for a couple of reasons
(s. discussion of stability of contrasts)
...
\todo[inline]{in general, the discussion explains how results fill gap
identified in intro provides caveats to the interpretation describes how
paper advances the field by providing new opportunities}
...
\todo[inline]{We demonstrate\dots; we discovered\dots; our data provide
evidence\dots}
...
objects \citep{epstein1998ppa, epstein1999parahippocampal} whereas a study using
spoken sentences reported mixed results \citep{aziz2008modulation}.
% what we wanted to test
Hence, we tested if the perception of spatial cues embedded in an auditory
narrative compared to non-spatial cues correlate with increased activation in
the PPA.
% ``how'' we did it
By using analogous statistical models for three different experimental
paradigms, we are able to compare results gained from a dedicated visual
localizer to results we gained from a movie and the movie's audio-only variant.

\todo[inline]{high-level summary/interpretation of results in one sentence here
already?}


\subsection{correlation of regressors}
\todo[inline]{could maybe put into method section to save space here}
...
Similarly to previous studies that used simplified stimuli to isolate the PPA,
we created traditional voxel-wise GLMs that modeled and contrasted
...
% intro
We performed whole-brain analyses on an individual and group level. On the group
level of both naturalistic stimuli, significantly increased activity in the PPA
correlates with spatial events compared to non-spatial events.


\subsubsection{AO stimulus}

\todo[inline]{Do Wilson (2008), Lerner (2011), Honey (2012), Silbert (2014) find
ISC in PPA using an auditory stimulus?}
% AO stimulus cluster location
More specifically for the primary contrast of the AO stimulus, group results
show spatially concise / restricted clusters in the anterior part of the PPA
group overlap taken from \citep{sengupta2016extension}.
% diff to Aziz (2008)
Unlike \citep{aziz2008modulation} we did not use whole sentences as events to
model a hemodynamic response but modeled an event from on- to offset of each
word that was giving the cue about the movie's missing visual content of a
scene.
% conclusion
our group results suggest that auditory spatial information does not
just modulate activity in the PPA (depending on other factory like familiarity,
XY; s. \citep{aziz2008modulation}) but correlates with bilaterally increased
activation.\todo{``modulate'' suggests causation}

% no task in Epstein
An early study \citep{epstein1998ppa} that compared results from a paradigm that
employed a (perceptual) judgment task of static pictures to a paradigm that did
not let participant judge the same pictures.
...
Nevertheless, our paradigm differs from previous studies because relevant
stimulus features were incidentally embedded in a continuous auditory
(or audio-visual) stream.
% conclusion
This embeddedness prevents participants from guessing the investigated
perceptual process by just being exposed to the paradigm and mimics experiencing
the world outside the lab more closely.
...
\paragraph{(non-existing) other cluster}
% whole brain analysis
Regarding confounding variables, we do not find significant clusters in e.g.
early auditory cortices despite performing a whole brain analysis on data gained
from a naturalistic stimulus paradigm.
% conclusion
This suggests that variance correlating with lower level auditory processes was
averaged (out) across trials and/or regressed out by implementing the nuisance
regressors.
% shit was averaged out we did not even know of
Further, unknown stimulus features we did not even annotate did not lead to a
variety of significant cluster across the brain that hamper interpretation of
results.
% conclusion
This provides further evidence that model-driven analyses can be run on data
from naturalistic stimuli to isolate brain activity correlating with specific
perceptual processes.
% superior lateral occipital sulcus
\todo[inline]{bilateral superior lateral occipital cortex}
...
\todo[inline]{from Epstein, 2008: Neuroimaging studies of human navigation most
commonly activate the posterior parahippocampal and retrosplenial cortices
[4–10], regions that also respond strongly during passive viewing of
navigationally relevant visual stimuli such as scenes and buildings [11–14]
\citep{epstein2008parahippocampal}.

% but RSC
Apart from the PPA, we find significantly increased activity in a functionally
defined scene-responsive region often referred to as the retrosplenial cortex
(RSC) in the primary contrast and consistently across the additional
contrast\todo{how many exactly}.

% location functionally defined
This functionally defined RSC which is not necessarily identical to the
anatomically defined retrosplenial cortex [26]
\citep{epstein2008parahippocampal} is located in the retrosplenial cortex, and
posterior cingulate region, near to the point where the calcarine sulcus joins
the parietal-occipital sulcus \citep{epstein2008parahippocampal}.
% anatomy of real RSC
...
\citep{epstein2008parahippocampal}.
% conclusion
Interpretation \& conclusion???\todo{cf.\citep{vann2009what}}


\paragraph{shortcomings of AO contrasts}
% no ``landscape'' category
Notably, our primary contrasts compared verbal cues regarding landmarks
(\texttt{geo}) and elements defining the geometry of a room (\texttt{groom}) to
non-spatial cues. While in the visual domain pictures of landmarks correlate
with significantly increased activity in the PPA pictures of whole landscapes
are considered to be the ``optimal'' stimulus type (cf. Epstein (1998, 1999,
2003, 2008).
% our se_new & se_old
Our auditory stimulus provided only a small amount of nouns that described the
landscape as a whole (e.g. ``in a football stadium'')  and these events were
pooled with other nouns that generally cued a switch to another scene into the
heterogeneous categories \texttt{se\_new} (e.g. ``Forrest in front of a
buffet'', ``in Greenbow'') and \texttt{se\_old} (e.g. ``at the bench'')
% further studies
...
investigate how homogeneous categories of vague verbal descriptions of a whole
scene differ from verbal descriptions of concrete landmarks.
...
% location
Clusters of the analysis of the AV stimulus span the group overlap of individual
PPAs from anterior to posterior and retrosplenial regions bilaterally.
% difference in location/size to AO and VIS
Clusters also extend into more posterior regions (occipital fusiform gyrus,
lingual gyrus, and intracalcarine cortex) \todo{s. neurosynth in fsleyes?}
...
% further studies
Hence, further studies focusing on visual perception should annotate and model
visual features of naturalistic stimuli more rigorously.
%
\todo[inline]{interpretation of clusters lateral occipital cortex?}
\todo[inline]{discuss that we DID NOT find clusters that could be attributed to
top-down / bottom-up attention, eye movements (FEF) (cf. corbetta2002control
or memory (which memory)?}


\subsubsection{stability of contrasts}
% intro
\todo[inline]{following logical structure is meh}
...
% it shows two thing
The results of the additional contrasts show two things. First, the
bilaterally increased activation in the PPA does not depend on accidental
findings but are stable across contrasts.
% second: decline in isolation power
Second, the ability to isolate clusters in the PPA declines nevertheless
depending on the amount of events available to the statistical model. More
importantly, isolation ability also declines depending on how well the used
events within categories matched on average the preferred stimulus type.

\paragraph{shortcomings: mixed events in AO categories}
% mixed categories
especially \texttt{se\_new} and \text{se\_old} (esp. in contrast 6-8) is often
mixed
% examples
e.g. "ein Arzt", "ein alter Film", "auf einer Straße", "im Fernsehen", "die drei
Jungs", "die zwei im Bett", "Jenny als Teenager", "Forrest vor einem Buffet",
"Kennedy", "Jenny kommt aus dem Wohnwagen ihrer Großmutter";
% hierarchical structure of annotation
Switches from one setting to another offered the least amount of events in both
stimuli. In the AO stimulus, this category shares events with nouns from other
categories that served to cue the listener about a change of the setting ->
words belong to more than one category but high-level category was used
% soundscape
Moreover, a change of the soundscape preceding the narrator's speech could also
cue listeners about a change of the spatial environment\todo{check contrasts
that use cuts across scenes but in the AO stimulus}
% conclusion
Hence, we assumed smaller probability of successfully isolating the PPA.
% concluding statement
In summary, the results of the additional contrasts suggest that increased
activation within the PPA is not just an accidental finding. Nevertheless, the
ability to isolate the PPA drops the less the amount of data are available and
the less the events averaged to built a category match the preferred stimulus
type processed in the PPA.
% hypothesis
Further, we hypothesize that not just semantic information but also the mere
changes of a soundscape that cues a listener about a setting's spatial layout of
could correlate with increased activity in the PPA
% crinkling
\todo[inline]{cf. arnott2008crinkling which find increased activations of PPA
to sound made by objects}


\subsection{individual analyses}

\todo[inline]{individual ``preference'' to pay attention to spatial infos (which
contradicts statement above that attention is not a prerequisite}

\todo[inline]{different level of alterness during paradigms}

\todo[inline]{different ``capability'' to process auditory spatial information}

\todo[inline]{individual differences: different contrasts should be used and
then the best used for individual rois}
...
Contrary to Sengupta et al. (2016), results were not archived by visually
expecting unthresholded $z$-maps and choosing a subjectively best fitting
threshold for each subject were archived but by using the same $Z$-value and
significance threshold for all subjects.
...
\todo[inline]{could also be discussed: in 1-3 subjects a different contrasts
work ``better''.}
% sub-04
In sub-04, the VIS yielded one right-sided PPA, the AV stimulus yielded one
right-sided significant cluster whereas the AO stimulus yielded two homologous
clusters in the PPA.\todo{what to do with that?}
% note
...
In summary, the individual results suggest that the group results are not driven
by a couple of subjects but still reveal individual differences.
% PPA just an example
The feature ``auditory spatial information'' is just one example of a stimulus
feature that can be annotated and used to build a model.
% auditory localizer
Thus, a complex but naturally engaging auditory stimulus like an audiobook
...
\todo[inline]{shortcoming to task to attend to spatial information}


\subsubsection{AV stimulus}
% intro
On individual level, mixed results for AV stimulus.  \todo[inline]{which
contrast works better in which subjects?} Individual differences when seeing a
whole picture with many features vs. serial auditory stream of information
providing incidental cues; spatial attention?
...
% connectivity
Aminoff et al. (2013) regarding Baldassano (2013): There is some evidence from
a human neuroimaging study examining functional connectivity of differences
between anterior and posterior regions of the PHC that the anterior regions are
...
patterns, with anterior PPA more strongly connected to regions in the default
mode network (including the parieto-medial temporal pathway) and posterior PPA
more strongly connected to occipital visual regions. We show that object
sensitivity in PPA also has an anterior–posterior gradient, with stronger
responses to abstract objects in posterior PPA. These findings cast doubt on the
traditional view of PPA as a single coherent region, and suggest that PPA is
composed of one subregion specialized for the processing of low-level visual
features and object shape, and a separate subregion more involved in memory and
scene context.
% Review in Baldassano
In the last several years, a number of researchers have suggested that PPA could
have multiple functional components. Differences in spatial frequency response
(Rajimehr et al., 2011), varying deficits resulting from PPA lesions (Epstein,
2008), PPA's overlap with multiple visual field maps (Arcaro et al., 2009), and
a clustering meta-analysis (Sewards, 2011) all hint at the possibility that PPA
may be comprised of at least two functionally distinct subunits along its
posterior–anterior axis. However, studies explicitly searching for a dis-
tinction between posterior and anterior PPA have failed to identify major
differences (Cant and Xu, 2012; Epstein and Morgan, 2012).
% anatomical studies in monkeys reviewed in Baldassano
In short, these macaque parahippocampal regions exhibit an anterior–posterior
gradient, with the anterior side most related to RSC and cIPL and the posterior
side most related to ventral visual areas.
% from Baldassano discussion
Our results demonstrate that human PPA exhibits a gradient in con- nectivity
along the anterior–posterior axis analogous to the gradient in connectivity
along macaque TH/TF/TFO. This connectivity gradient was also paired with a
functional gradient of sensitivity to scene and abstract object stimuli. These
results present a challenge to current models of PPA function which assume that
PPA is functionally homogeneous, and demonstrate that anterior and posterior PPA
connect differentially to two distinct cortical networks.
% but not independent
Note that, although our data suggest that PPA might contain identifiable
subregions, these subregions should not be considered as completely independent
modules. Both subregions activate selectively to scenes, and the parahippocampal
region (at least in macaques) is densely self-connected (Suzuki, 2009), implying
that these subregions cooperate to build a complete representation of a scene.
...
overarching goal of scene understanding. We discuss some possibilities for the
functional roles of the subregions below.
% posterior PPA
...
in the posterior portion. In other words, posterior PPA may be more visually
responsive than anterior PPA.
% LLA
...
2007), and data from Aminoff et al. (2007) has suggested that an anterior
portion of PPA may be involved in recall based on spatial context. Our results
are con- sistent with anterior PPA playing a more central role in memory than
posterior PPA, given anterior PPA's connectivity with the DMN.
%
PPA is known to represent global scene properties such as spatial expanse
(Kravitz et al., 2011a; Park et al., 2011), and to construct global scene
representations which are not predictable from responses to signature objects
(MacEvoy and Epstein, 2011). Anterior PPA's connectivity to cIPL and RSC, along
with its lower sensitivity to abstract objects, suggest that it may be more
concerned with these types of spatial and non-object-based scene properties than
posterior PPA. Future research contrasting global and object-based properties of
scenes, however, would be necessary to test such a hypothesis.
%
...
% summary
Unraveling the functions of the PPA has proven to be a challenging problem,
given the region's involvement in a variety of scene perception and navigation
tasks (Epstein, 2008). Our results imply that a complete model of PPA's
functional properties must account for the differences in connectivity and
function between anterior and posterior PPA. Although the precise roles of PPA's
subregions are yet to be determined, our results and previous work suggest that
posterior PPA is concerned primarily with perception of low-level visual
features and object shape, while anterior PPA is involved in memory and global
...
PPA; Epstein, 1998). We of course do not argue that the PHC is not sensitive to
...
Aminoff et al. (2013). It was critical to compare contextual processing of
...
parahippocampal\dots). Activity within the PHC, however, was distributed along
...
\todo[inline]{Aminoff discusses contextual associations in visual spatial
processing, (episodic) memory; that could be discussed but the paradigm and
contrasts do not provide material to say much about it other than wild
speculations}

% Aminoff (2013)
It has been shown that the processing of strong, long-term, contextual
associations elicits activity within the PHC, as well as the RSC (which includes
regions of the retrosplenial cortex, extending into the posterior cingulate
cortex, and the precuneus), the medial prefrontal cortex (MPFC), and the
transverse occipital sulcus (TOS) (Figure 2) [2,3,59–67].
...
Naturalistic language processing can be approached with the analysis methods
constructed to handle well-controlled experiments
...
level; increased compliance in individual diagnostics in ``special'' populations
where compliance is more difficult to archive (pediatric or psychiatric)
