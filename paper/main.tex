...
The PPA is located in the posterior parahippocampal gyrus including adjacant
regions of the fusiform gyrus \cite{epstein1998ppa} at the boundary to the anterior lingual gyrus \citep{epstein2008parahippocampal}.
...
verstanden habe, hat sie den Unterschied zwischen PPA und FFA angeguckt, aber
diesen Unterschied im Kontrast zu object-related Sentences (dortige Fig 3)}
% Aziz' stimuli
...
% disadvatage 2b: no individual level
...
audio-only stimulation\todo{``audio-only'' sounds meh}.
% Huth (2016)
\todo[inline]{but Huth et al. (2016) implicitly regarding audio-only; but they
do not discuss the PPA; moreover, they do voxel-wise encoding which doesn't say
...
% used studies
\todo{this is just loveless listing so far} The same subjects were
% AO
...
groom & elements defining geometry of local space & \aoGroomAll & \aoGroomI & \aoGroomII & \aoGroomIII & \aoGroomIV & \aoGroomV & \aoGroomVI & \aoGroomVII & \aoGroomVIII \tabularnewline
...
\todo[inline]{Epstein (2008): response to arrays of objects drawn from the rooms
    but displayed on a blank background is quite weak [11]}
...
\subsection{intro}
\todo[inline]{goal and what we did should probably in result section}
% study goal
...
audio-description to create GLMs of hemodynamic activity resembling analyses
of previous studies using simplified stimuli.

\todo[inline]{summary of results like ``Our results demonstrate that\dots''}


\subsubsection{correlation of regressors}
% stimuli not designed for research purposes
Both stimuli were not designed to for research purposes detection power in a
fMRI experiment) but to entertain their audiences.
% why we created the correlation matrix in the first place
We first tested for correlation of stimulus features because a high degree of
shared variance lowers detection power.
% summary of results
Results show only minor to no correlations of regressors across all stimulus
segments.
% statement
Hence depending on the stimulus features of interest, it was feasible to use the
annotated stimulus features for our research question.
...
% no task
Compared to \citep{ocraven2000mental} participants of our study were not
instructed to form a vivid mental image of the stimulus but to ``enjoy the
audiobook''. This procedure resembles experiencing the world around us in
...
by merely being exposed to the stimuli / experimental design. This suggests
spatial information is processed in the PPA despite a lack of visual
attention.\todo{bold claim here}
% comparison to Wolbers
Similarly to \citep{wolbers2011modality} who found increased activation in the
PPA when blind study participants haptically explored scenes made out of LEGO
blocks compared to other objects, our results support the hypthesis that
increased activation in the PPA does not necessarily rely on creating a vivid
mental image.
% (visual or auditory) spatial attention
Results further suggest activation does also not rely on deliberately paying
auditory attention to spatial information embedded in a complex/rich auditory
stimulus.\todo{kinda bold statement}
% others
...
% RSC
Epstein (2008): Neuroimaging studies of human navigation most com- monly
activate the posterior parahippocampal and retro- splenial cortices [4–10],
regions that also respond strongly during passive viewing of navigationally
relevant visual stimuli such as scenes and buildings [11–14]. Consistent with
these findings, damage to these areas often leads to wayfinding deficits [15].
These results indicate that para- hippocampal and retrosplenial cortices are key
nodes of the neuronal network that support spatial navigation in humans. But
what are the specific functions of each of these nodes?
% location
A second focus of cortical activity during navigation tasks is found in the
retrosplenial cortex-posterior cingulate- medial parietal region, near to the
point where the calcarine sulcus joins the parietal-occipital sulcus.
Retrosplenial cortex (BA 29 and 30) adjoins and is partially encircled by the
posterior cingulate (BA 23 and 31) [51–56]; consequently, these labels are often
used somewhat inter- changeably when describing the locations of functional
activations. Because of these ambiguities, I use the term retrosplenial complex
(RSC) to refer to this functional- defined scene-responsive region, which is not
...
imagination of navigation through familiar environments [8]. In contrast to the
PPA, in which place familiarity effects are usually quite small, RSC responds
50% more strongly to familiar than to unfamiliar places [23,57], indicating that
RSC is involved in recovery of long-term spatial knowledge about familiar
environments. Further evidence for this idea comes from recent studies reporting
that RSC activity was signifi- cantly greater when subjects reported the
location of a campus scene than when they simply reported whether it was
familiar or not [24] (Figure 3a) and that RSC activity during learning of a
virtual-reality town corresponded to the total amount of survey knowledge
acquired [58] (Figure 3b; see also Ref. [59] for similar results in mice).
% summary
The PPA and RSC seem to have distinct but complemen- tary roles during spatial
navigation. The PPA encodes a representation of the local scene that enables it
to be remembered and subsequently recognized, whereas RSC supports mechanisms
that enable one to orient oneself within the broader spatial environment and to
direct one’s movement towards navigational targets that are not cur- rently
visible

\paragraph{shortcomings of AO contrasts}
% less ``scenes'' as such but more isolated ``landmarks

%  se_new & se_old is often belongs to another (``hierarchical'' lower)
%  category; interesting who PPA responds to words describing a scene as a whole
%  (the gist) vs. landsmarks a scenes consists of
...
% location
Clusters of the analysis of the AV stimulus span the anterior, middle and
posterior part of group overlap mask/roi.
% difference in location/size to AO and VIS
Compared to cluster of the AO stimulus, they further extend into more posterior
regions.\todo[inline]{which exactly? check fsleyes again; known functions of
these areas -> should be explainable by confounds in movie frames}
...
% Epstein (2008)
In particular, anterior PHC might do more than just scene recognition; it might
also have a more general role in spatial memory encoding [94].
...
\todo[inline]{here, A LOT could be discussed; possibilities: individual
    ``preference'' to pay attention to spatial infos (which contradicts
    statement above that attention is not a prerequisite; different level of
    alterness during paradigms; different ``capability'' to process auditory
spatial information (maybe correlating in behavior differences in navigational
tas in navigational tasks}
...
Contrary to Sengupta et al. (2016), results were not archieved by visually
expecting unthresholded $z$-maps and choosing a subjectively best fitting
threshold for each subjectiwere archieved but by using the same $Z$-value and
