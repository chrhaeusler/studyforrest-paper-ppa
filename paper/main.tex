...
For both stimuli we created several contrasts that varied the amount of
...
% but
Nevertheless, results also suggest that ``localization performance'' depends on
the amount of available data and how well the feature space is modeled.
% example
...
change of a setting (\texttt{se\_new} or \texttt{se\_old}) to categories that do
not cue a change of a setting (contrasts 2, 3, 6, 7) show significant clusters
in (primary and secondary) auditory cortices.\todo{``poor job of nuisance
regressors?}
...
% possible reason
This could be attributed to a possible change of the soundscape when the
narrative switches to a new setting.
% concluding statement
