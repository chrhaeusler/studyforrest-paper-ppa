...
% AV events
For the AV stimulus, we created regressors correlating with annotated movie cuts
...
for that segment.  \todo{wenn ich "null regressor" nehme, darf ich den ganzen
    Run, glaube ich, in der 2nd level Analyse gar nicht benutzen? Hatte, glaube
    ich, Jeanette Mumford in einem ihren Videos mal gesagt}
...
of the soundscape after cuts, and to create contrasts of cross-modal negative
control.
...
% Reference to table
An explanation and counts of the eleven speech-related event categories,
...
can be seen in Figure \ref{fig:reg-corr}.\todo{correct colors on horizontal
line; drop control regressors from figure}
...
% intro
For the AO stimulus, we created eight $t$-contrasts that aimed to isolate the
PPA and five $t$-contrasts for negative control purposes (see Table \ref{tab:ao-contrasts}).
% PPA contrasts
\todo[inline]{setting-related regressors have small event count compared to
geo/groom; probably more important in discussion}
...
Furniture was not included in one of the contrast because it can be considered
as an object or geometry-defining structure of a room.

% negative control contrasts
Contrasts 9-13 correspond to the contrasts 1-5 of the AV stimulus and served as
negative controls because increased activation in the PPA could also be
correlated with moment of cuts and a possible change in the soundscape of a
scene.
...
\todo[inline]{change stability of contrast plot from 6 to 5 COPES (line 40 in
script)}
...
and five $t$-contrasts for negative control purposes.
% no control for frames content
Notably, this was done without an annotation of the visual content depicted in
the frame after each cut and statistically controlling confounding variables.
% cinematographic rationale
The cinematographic rationale was that movie tend to establishing the setting
(\texttt{vse\_new}) and the spatial relationships within a setting
(\texttt{vpe\_new}) at the beginning of a movie. Later in the movie, the field
sizes of movie shots tend to decrease when switching back to already established
settings (\texttt{vse\_old}). Shots within a recurring settings tend to shift to
depicting persons and objects (\texttt{vpe\_old}) more relevant to the evolved
narrative \citep{brown2012cinematography, mercado2011filmmakers}.
...
% PPA masks
PPA ROI masks for individual subjects and a PPA group mask of individual PPAs overlapping were created from data provided by \citep{sengupta2016extension}.


\section{Results}

\todo[inline]{I consider tables of significant clusters for every contrast as
overkill. One contrast, the ``primary contrast'', appropriate.}
...
% LOC
Four contrasts show signficant bilateral clusters in laterial occipital cortex
...
\begin{figure*} \centering
    ...
    \caption{Stability of contrasts. Overlap of significant clusters (Z>3.4;
        p<.05, cluster corrected) of contrasts created to isolate the PPA. Blue:
        contrasts 1-7 (see Table \ref{tab:ao-contrasts} for the
    audio-description.}. Red: contrasts 1-5 (see Table \ref{tab:av-contrasts}
    for the audio-visual movie. Black: outline of overlapping individual PPA
    ROIs \citep{sengupta2016extension}.  \label{fig:stability-slices}
\end{figure*}


\subsection{primary contrasts}
See Figure \ref{fig:group-slices}.

AO stimulus; See Table \ref{tab:res-ao-group1}

AV stimulus; See Table \ref{tab:res-av-group1}
...
The first brain structure given contains the voxel with the maximum Z-Value, followed by brain structures from posterior to anterior, and partially covered areas (l. = left; r. = right; c. = cortex; g. = gyrus).}
...
\begin{figure*} \centering
    ...
    \caption{Mixed-effects group-level (N=14) GLM results.  Significant clusters
        ...
        Blue: the $t$-contrast comparing
        geometry-related nouns to non geometry-related nouns spoken by the
        audio-description's narrator (geo, groom > all non-geo).
        Red: audio-visual movie (red).}
