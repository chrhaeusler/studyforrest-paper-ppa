...
start of the recording for the second half of the movie.\todo{references to bib}
...
overkill. Tables for just the ``primary contrast'', seems more appropriate}
...
Seven of eight contrasts show bilaterally significant clusters in the
retrosplenial region (and ventral precuneus).
% statement/answer
...
contrasts}
% confounds
but confounds: right superior temporal gyrus (4 contrasts); left superior
temporal gyrus (3 contrasts); some prefrontal clusters that show up in two
contrasts
...
model a hemodynamic responses but modeled from on- to offset of single words
...
% no task in Epstein
An early block-design study \citep{epstein1998ppa} compared results from a
paradigm that employed a (perceptual) judgment task of static pictures to a to
the same paradigm but without that task.
...
Nevertheless, our paradigm still differs from previous studies because the
relevant stimulus features were incidentally embedded in a continuous auditory
stream.
% conclusion
...
These categories were actually heterogeneous: they rarely contained whole (but
also vague) descriptions of landscapes (e.g. ``in a football stadium'',
``Forrest is running through the jungle''), mostly landmarks or buildings,
and they also contained non-spatial cues.\todo{repeat examples from method
section}.
% further studies
...
investigate if categories of vague verbal descriptions of a landscape as a whole
lead to differential hemodynamic activity in the PPA compared to more concrete
spatial cues.
...
lingual gyrus, and intracalcarine cortex) \todo{what overlaps with group ROI?}
...
\todo[inline]{concluding statement to cross-modality of results?}
...
% PPA AV
Regarding contrasts of the AV stimulus, x of y contrasts show significant
...
\subsubsection{localization performance and noise}
% overall good isolation;  whole brain analysis and not many random clusters
Despite performing a whole brain analysis on naturalistic stimuli with many
uncontrolled stimuli we do not find many random clusters in single contrasts or
even systematically across contrasts\todo{good god but that's essentially it}.
...
results.
% movie: attention, and shit
\todo[inline]{where are no clusters? e.g. top-down / bottom-up
attention, eye movements (FEF) (cf. corbetta2002control) or memory (which
memory)?}

% BUT: amount of data
Nevertheless, localization performance depends on the amount of available
data\todo{give examples of contrast using less events in AV and AO}.
% BUT: match of the stimulus type/averaged events
Further, localization performance is also influenced by how well events
within categories matched on average the preferred stimulus type.
% AV: frame content
In the AV stimulus, we averaged different kind of movie cuts regardless to the
movie frames content.
\todo[inline]{how well spatially constrained are the clusters?}
% AO: stimulus categories
Moreover, nouns of the narrator cueing a switch to another setting are often
forestalled by changes of the soundscape.\todo{cross-modal control contrasts;
all negative results at moment of cuts but vse\_new > pe\_old): right RSC.}
% arnott
...
% AO: superior temporal cortex
Nevertheless in four of nine contrasts of the AO stimulus, we find significant
clusters in superior auditory cortices (including early auditory
cortices?)\todo{which exactly}
% conclusion
