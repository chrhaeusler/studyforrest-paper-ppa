...
Since the time-series of modeled hemodynamic activity were based on events in
stimuli that were not designed for research, we then tested if time-series were
correlated (i.e. share variance) leading to low detection power in subsequent
analyses\todo{pick your favorite}.
% fixed time course; detection power
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Since we annotated events in fixed time courses that could not experimentally
manipulated, the modeled time-series [of hemodynamic activity] based on these
events could be correlated leading to shared variance and thus low detection
...
across stimuli (\texttt{fg\_ad\_rms} and \texttt{fg\_av\_ger\_rms}, r=.7635;
\texttt{fg\_ad\_lrdiff} and \texttt{fg\_av\_ger\_lrdiff}, r=.7749). This was
expected and served as ``sanity check'' since the audio-track of the
...
\todo[inline]{I created the part "design matrix" cause following parts are
usually written in the "regressor paragraph" in publications but do not really
fit in the subsection above}
...
% null regressors
If a stimulus segment did not contain an event of a category (e.g. no event of
\texttt{vpe\_old} in segment 3 of the movie; cf. Table \ref{tab:ao-events}), a
null regressor was chosen for that category in that segment.
