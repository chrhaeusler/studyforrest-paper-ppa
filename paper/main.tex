...
\subsubsection{Events (based on annotations)}

\todo[inline]{mention low-level confound annotations here or (as it is now) in
modeling}
...
physician'').
% how it was handled
These nouns were flagged with both category during the initial annotation
...
(\texttt{se\_new} or \texttt{se\_old}).\todo{if you know what I mean}
% pooling of categories
Lastly, some categories that were semantically similar categories that offer
only a small amount of counts were pooled resulting in 11 final categories (see
Table \ref{tab:ao-events}) that were used as event categories [to model
different hemodynamic responses].
...
\begin{table*}[h!]
    \caption{Categories and rules to categorize the descriptive nouns spoken by
        the audio-description's narrator. Some of these initial 18 noun
        categories were pooled resulting in 11 event categories that served as
        a basis to build the regressors of the GLM
