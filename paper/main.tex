...
In this study, we explore different ways to operationalize spatial perception\todo{not sure if this is just perception} in
14 subjects that were watching an audio-visual Hollywood movie and listened to
the movie's audio-description (studyforrest.org).
% auditory localizer?
Focussing on the audio-description, we test if an auditory narrative
could localize a ``visual area'' in individual subjects.
% GLM
For each naturalistic stimulus, we model hemodynamic activity via several
general linear model (GLM) $t$-contrasts that are based on annotations
...
%\citep{sengupata2016extension}.
% results: group AV
On a group level, results for a visual movie stimulus show significantly increased hemodynamic
activity spatially overlapping with a traditionally localized PPA but also extending into earlier visual
cortices.
% results: group AD
Results for the audio-description stimulus reveal significant activation restricted to the
anterior part of the PPA.
...
Our results provides further evidence that the PPA can be divided into
functional subregions.
% localizer substitute
An auditory narrative might substitute a visual paradigm to localize the PPAs
anterior part in visually impaired subjects.\todo{very bold claim to be written
in the abstract}

\end{abstract}
\todo[inline]{RSC \& LOC are not mentioned in the abstract (anymore)}
...
\begin{itemize}
    \item structure of last paragraph might be meh: methods/our approach
-> hypotheses -> results; readers might assume HARKing?
\end{itemize}
...
\begin{itemize}
    \item Zwischenüberschriften rausnehmen, wenn Struktur so in Ordnung
    \item revise captions of Table 1 + 3 (regressors based on annotation of
    the other FG stimulus are now dropped from the table but mentioned in
    section "statistial model / contrasts")
\end{itemize}
...
\begin{itemize}
    ...
    \item ggf. berichten welcher der drei VIS Kontraste für welche VP genommen
    wurde
    \item location of tables \& figures is pretty confusing at the moment
    \item length of report of other reliably significant cluster across
        contrasts and report of control contrasts depends on if/how long they
        should be discussed
\end{itemize}
...
\noindent Discussion
\begin{itemize}
    \item umschreiben gemäß neuer Schwerpunktsetzung in Intro \& Method-Section
    \item \citep{bartels2004mapping, hasson2004intersubject}
...
categories occurring in spoken language.\todo{phrasing to delineate from Huth
(2016); imo voxel-wise semantic modeling says nothing about
...
Contrasts vary in HOW WELL / HOW MUCH DATA.\todo{rewrite if it should be
mentioned}

% hypo a
We hypothesize[d] that whole-brain analyses will/would reveal increased
hemodynamic activity spatially constrained \todo{i.e. not activity all over the
place} to the PPA as functionally defined by a visual localizer experiment using
pictures done with the same subjects \citep{sengupta2016extension}.
...
visual localizer.\todo{interpretation?}
...
\todo[inline]{the topic anterior vs. posterior is mentioned at the end of the
intro for the first time; might be "surprising" but that is in line how the
story went (it's mentioned in the abstract though)}
...
\citep{haxby2011common}.\todo{I guess they only report the ones they
designed themselves in detail? -> check again}
%
...
% AV regressors
\begin{table*}[h!]
    \caption{Overview of annotations and event counts in the whole audio-visual
    movie and individual stimulus segments.
...
% AD regressors
\begin{table*}[h!]
    \caption{Overview of annotations and event counts in the whole
    audio-description and individual stimulus segments.
...
\paragraph{Confounds for both}

\todo[inline]{Die Events für die low-level confounds habe erstmal hier
beschrieben; btw: Das Verschieben des Sounds in den Segmenten spiegelt sich
wahrscheinlich auch in den unterschiedlichen Counts der Events je Segment wider
(AV Bild vs AV Sound vs AO Sound)}
...
\subsubsection{Hemodynamic Modeling}

% movie events
The events of the movie cut related categories and \texttt{no\_cut} events were
modeled as impulse events.\todo{actually, it was a box car of 200 ms starting
with the cut}.
%
...
jetzt lange darüber gebrütet: Es gibt also kaum eine Korrelation zwischen AD und
AV?; Also, außer bei fg\_av\_ger\_lr\_diff (wobei da keiner weiss, was das
bedeutet); COH: what is better: reference to table as it is now (more precise,
but harder to read) or use non-cryptic words in the figure?}
...
% level 1 and 2; similarly to the reviewed studies that employed designed
% stimuli; we "based on stimulus annotations"
We then tested our modeled hemodynamic responses by conducting standard
two-level general linear model (GLM) analyses to create subject-specific results
($Z$-maps) across the 8 segments for every subject and each stimulus.
% level 3
...
% AV design matrix
For the first level analysis of the AV stimulus, the modeled hemodynamic
responses related to all movie cuts (including \texttt{no\_cut}) and six
low-level perceptual confounds were included as regressors in the design matrix
(s. Table \ref{tab:av-events}).
...
audio-description's narrator to cue switches to another setting (categories
...
In analyses of both stimuli, a null regressor was chosen for a event category in
a segment, if that segment did not contain an event of the category (e.g. no
event of \texttt{vpe\_old} in segment 3; cf. Table \ref{tab:ao-events}).
...
\paragraph{PPA Contrasts}

In this study, we investigated if increased hemodynamic activty in the PPA that
is usually defined by contrasting blocks of pictures can also be revealed by
contrasting selected stimulus features that we annotated in a audio-visual movie
and its audio-description.
% intro
To test the robustness of our approach, we operationalized the perception of
...
\citep{sengupta2016extension}).\todo{maybe add note regarding Berman (2010);
but they did FFA}
...
regressor (texttt{vno\_cut} to cut-related regressors \ref{tab:av-contrasts}).
% AV: narrator
One contrast (\texttt{vs\_new} > \texttt{vpe\_old}) was created by
contrasting nouns spoken by the narrator.
\todo[inline]{why was done? narrator and cuts to other setting are "somewhat"
correlated; and now what? -> cf. results and discussion}
% AD stimulus
...
change of the soundscape also giving an auditory cue of switching to another
setting.
...
\paragraph{GLM computations \& stuff}
...
sagen: bei FFA/PPA etc sind flatmaps/inflated brains standard; COH: imo it is
...
activity in the PPA that was defined using a visual localizer experiment
...
create GLM contrasts similar to previous studies that used static pictures.
% group level
...
% individual level
On the level single subjects, we also assess the performance of a naturalistic
auditory stimulus as a substitute for a visual paradigm to localize the
PPA.\todo{``localize''}
% NeuroVault
...
% intro
First, we tested if results of a whole-brain analysis based stimulus annotations
would lead to increased hemodynamic activity in the PPA.


\subsubsection{group}
% AV intro
First, we analyzed data from the audio-visual movie offering visual but
ecologically more valid stimulation.
% AV primary contrast
...
% AD intro
Second, we analyzed data from the movie's audio-description offering exclusively
...
\todo[inline]{imo no need to mention it here but mention in discussion that
results are not driven by outliers when discussing ``localization'' performance}
...
stimulus can/could localize ``visual area'' in individual subjects and thus
possibly substitute a visual paradigm using blocks of pictures.

\todo[inline]{following is probably the part that Simon considered as en
passant}

\todo[inline]{if we here report procedure of Sengupta as detailed as
it is now, we probably need to explain the differences between strict, relaxed,
simple in more detail in the method section and not just reference to Sengupta}
...
\citep{sengupta2016extension}).
% Sengupta: procedure a)
Starting with the most conservative contrast (strict set) and a threshold of t =
2.5, we looked for clusters with at least 20 voxels (using AFNI).
%
We titrated [?] the threshold in the range of [2, 3] until we found an isolated
cluster for the PPA.
% Sengupta: procedure b)
If a cluster was not found or not isolated, we used a contrast from the relaxed
set, or finally the simple set, and repeated the process until we found a
cluster that matched the expected anatomical location based on literature
\citep{epstein1998ppa}.

% naturalisti stimuli: intro
In the current study, time series of all subjects were analyzed both in subject
space and group space.
% subject space -> NeuroVault
Results of the analyses that were performed in each subject's [subject-]space
(unthresholded $z$-maps and thresholded $z$-maps, Z>2.3, p<.05 cluster
corrected) can be found at
\href{https://neurovault.org/collections/KADGMGVZ/}{NeuroVault}. \todo{still
private collection}
% here report in group space
For better comparions of subjects, we here report results from the analyses
performed in group space. % ($z$-maps; Z>3.4, p<.05 cluster-corrected).
% one contrast to localize them all; "...and Z>3.4 for all"
Contrary to the procedure in \citep{sengupta2016extension}, we used the same
contrast of each naturalistic stimulus and the same threshold (Z>3.4, p<.05
cluster corrected) for all subjects.

% AV
Results of the primary AV contrast yielded statistically significant bilateral
clusters in five subjects, unilateral right clusters in six subjects (of
which one subjects yielded a unilateral cluster in the visual localizer), and a
unilateral left cluster in one subject.
...
Results of the primary AD contrast yielded stastically significant bilateral
clusters in nine subjects, and one cluster in the left hemisphere of sub-09,
...
\subsection{Robustness of PPA across contrasts: AV \& AD}

\todo[inline]{tables of significant clusters all contrasts = overkill!? Link to
NeuoVault is added; sign. clusters of all contrasts are listed in the comments}
...
are robust across differently designed contrasts.\todo{concluding statement
often feels like interpretation}
%
\todo[inline]{figure caption: mention / explain other clusters}

% figure group results of robustness across all AD & AV contrasts
\begin{figure*}[h!]
\centering
...
\subsection{Non-PPA clusters across contrasts}

% precuneus = retrosplenial cortex ?
\todo[inline]{Following is a list; report depends on the discussion I guess;
anyway: clusters are not ``random'' but make sense which speaks for the use of
...
\item lateral occipital cortex: 6 contrasts (4 bilateral, 2 unilateral =
...
\subsection{control contrasts (AV \& AD)}

\todo[inline]{contrasts are mentioned in the method section; shall results be
reported (and later discussed)?}
...
parietal lobe. Compare Figure \ref{fig:reg-corr}: nouns that cue change to a new
...
"localizer vs naturalistic" im vordergrund stehen, AD vs AV dann eine sehr
...
\todo[inline]{concluding statement regarding AV vs AD and cross-modality of
results?}
...
%
Further, the incidental occurrence of modeled features prevents participants
...
\todo[inline]{individual results should be incorporated into above section;
depends on long discussion of individual results will be}


\subsubsection{AV stimulus}

\todo[inline]{was not aim of study; results of primary AV contrasts are in plots
    if individual; Bland-Altman-Plots focus on primary AD contrast alone}
...
\todo[inline]{dunno if this is meaningful: z-scores in the anterior part of
    PPA are higher in AD contrasts than in contrast of localizer}
...
based on cinematography => the stimulus changes over time; AV analysis exploits
confound structure}
...
% focus of study
The focus of the current study was to investigate if significantly increased
...
jungle'') but mostly landmarks or buildings, and also non-spatial cues (e.g.
``Jenny as a teenager``).
% further studies
