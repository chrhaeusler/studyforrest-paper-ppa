...
\todo[inline]{why naturalisticc stimuli are so cool: sinkusare2019,
eickhoff2020}
...
\todo[inline]{Nach wie vor sind Teile aus den Originalstudien ausführlich}

\todo[inline]{Reihenfolge der subsections mag ``inkorrekt'' sein.}

% intro
We used data from three publicly available datsets, all being part of the
studyforrest dataset (\href{http://www.studyforrest.org}{studyforrest.org})
which has already been used by other research groups for independent research
questions.
% used studies
...
study) \citep{sengupta2016extension}, a dataset aready used by
...
\todo[inline]{inner join von AO \& AV: N=14; AO: N=19; Exklusion von VP10 kommt
unter ``Data''}
% AO study
In the AO study, 19 German native speakers (all right-handed, age 21–38 years,
mean age 26.6 years, 12 male) listened to the German audio-description
...
listeners on Swiss public television.
% AV study
In the AV study, 15 participants (21–39 years, mean age 29.4, six female) of the
prior AO study watched the audio-visual movie with dubbed German audio track
\citep{ForrestGumpDVD}.
% VIS study
In the VIS study, the same 15 participants took part in a six-category
block-design visual localizer.
% participants' health
...
informed consent for their participation and for public sharing of obtained
data in anonymized form. The studies had prior approval by the Ethics Committee
of Otto-von-Guericke University of Magdeburg, Germany.
...
additional male describes essential aspects of the visual scenery when there is
no off-screen voice, dialog, or other relevant auditory content.
...
the narrator to describe the movie's missing visual content.
...
furniture). See Table \ref{tab:descr-nouns-counts} for a complete overview of
all cateogries [+++rules, counts, examples].
% procedure
A preliminary annotation was performed by one person according to the rules (see
Table \ref{tab:descr-nouns-rules}). Minor corrections due to incorrectly applied
rules were by the author.
...
\begin{table*}[t]
    ...
    cateogries, annotation rules and examples (given in English).}
...
furniture & moveable objects insides and outsides & bench, bed, table, chair
\tabularnewline
geo & immobile landmarks & building, tree, street, alley, meadow, cornfield, river \tabularnewline
...
object & countable entity with firm boundarie & telephone, car \tabularnewline
persons & concrete persons of unknown sex & hippies, patients \tabularnewline
setting\_new & setting mentioned for the first time &  on a ``bridge'', on an ``alley'', on ``campus'' \tabularnewline
setting\_rec & recurring setting & at the ``bus stop'' \tabularnewline
++ & nouns, adverbial adjectives and adverbs & in the ``evening'', it's ``daytime'', ``later'' \tabularnewline

\bottomrule
\end{tabular}
\end{table*}

% counts Tabelle für descriptiven nouns
\begin{table*}[t]
...
demographic information (and familiarity with the audio-description or movie).
% AO & AV: instructions
...
four segments in two fMRI sessions respectively. Between sessions, participants
left the scanner for a break with a flexible duration. Structural images were
obtained during the first study on a day different from the fMRI session.
...
\todo[inline]{Ist richtig, dass auditory stimulation in beiden Experimenten die
gleiche war?}
...
positioning procedure was identical in the AV und VIS study and yielded a
congruent geometry across paradigms.

% no. of volumes
A total of 3599 volumes were recorded for each participant in each of the
naturalistic stimulus paradigms (451, 441, 438, 488, 462, 439, 542, and 338 volumes for segment 1–8 respectively).
...
\todo[inline]{motion correction \& Templateerstellung: cite sengupta2016 where this was first done and described, ie same
image/data was used here => dort finde ich nur links
...
\todo[inline]{Satz zum Downsampling der 7T Daten? => check github repo}
...
% exclusion of VP 10
Data of one participant was dropped to due to invalid distortion correction
...
\todo[inline]{Nouns, die eigentlich zu einer anderen Kateogrie als setting\_new
    or setting\_old gehören, aber einen Settingwechsel anzeigen, werden als
Event eines der Settingwechsel zugehörig gezählt.}
...
\todo[inline]{in der Abbildung der Regressoren-Korrelationen sind jetzt alle
    Regressoren drin; imo könnte man ein paar herausnehmen (die zur Kontrolle
    benutzen Filmschnitt-Regressoren in der AO-Analyse, die zur Kontrolle
    benutzen Descriptive-Nouns Regressoren (namentlich se\_new und se\_old), und ggf. [ein paar der] low-level features nuissance regressors.}
...
\begin{figure*} \centering
    \includegraphics[width=\linewidth]{figures/regressor-corr} \caption{Pearson
        ...
        \ref{tab:av-events}). Regressors were convolved with FSL's
        ``Double-Gamma HRF'' as a model of the hemodynamic response function,
        temporally filtered with the same high-pass filter (cut-off
