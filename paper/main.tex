...
\todo[inline]{Simon als Autor adden}

\todo[inline]{zweiten Satz: PPA ist die Region im ventral stream, die mehr auf
    static landscapes denn Gesichter reagiert. Ansonsten klingt es so, als ob
    PPA unabhängig von der Funktion definiert ist; dritte Satz ist irgendwie
    falsch konstruiert (narrative... audiovisual movie); ``significant increased
    activation in PPA'': PPA a priori durch localizer definiert? Oder ist der
    increase die Definition der PPA; Beschreibungen sind auf der einen seite
    sehr detailliert, aber der Vergleich mit dem localizer wird nicht explizit
erwähnt; Clou der ganzen Geschichte ist doch: Man braucht keinen separaten
Localizer, oder? Das kommt bisher nur bedingt rüber}
...
\todo[inline]{Den Start finde ich etwas komisch. "Cognitive neuroscientists use"
klingt sehr nach einer aktiven distanzierung}

\todo[inline]{Ich würde am Start definitiv kurz in die funktionelle
    Spezialisierung des visuellen Systems und die Organisation des ventralen
Streams einführen. Die kurze Darstellung der PPA ist dann aber gut}
...
\todo[inline]{ie "Kritik" an bisherigen PPA Studien finde ich insgesamt OK,
wobei nicht ganz klar wird, was genau das schlechte an "conceptualized" ist. Das
löst sich erst am Ende mit der "ecological validity" einigermaßen auf}

\todo[inline]{Aus meiner Sicht könnte die Motivation für die aktuelle Studie
    noch stärker werden. Du nennst zwar einige Schwächen, aber dann geht es sehr
    unvermittelt in Details der Studie. Meine Präferenz ist es eher, nochmal
    ganz klar zu sagen, warum es jetzt diese Studie braucht und was sie zum Feld
beitragen soll}

\todo[inline]{Damit zusammen hängend: Das Konzept der aktuellen Arbeit (place
information aus einem ongoing context zu verwenden) und warum das die Probleme
löst, würde ich auch noch etwas klarer darstellen. Du setzt zu viel voraus}

\todo[inline]{Das "then we analyzed publically available data" ist sehr
verwirrend, da es doch davor auch um StudyForest geht}
...
\todo[inline]{as mit dem "already used by" verwirrt eher}

\todo[inline]{Der StudyForest datansatz ist gut und knapp beschrieben. Die
Stimuli im VIS datensatz brauchen aber glaube ich etwas mehr erklärung}
...
\todo[inline]{merge Stimuli and ``Procedure'' (s. below)}
...
\todo[inline]{Procedure (2.4.) ist nicht zwingend nötig bzw nicht als
separater Abschnitt}
...
\todo[inline]{Ich würde vorschlagen, dass Du Dich auf die Regressoren / Events
    konzentrierst, die hier wichtig sind, um die Komplexität zu reduzieren. Die
    aber dafür besser erklären (was heißt "unfamilar perspective"? Ab wann ist
ein setting "familiar"?}
...
\begin{table*}[t]
    \caption{Overview of events to build the 14 regressors for the analysis of
        ...
        \texttt{fg\_av\_ger\_lr} (difference in mean luminance of left and
        right half),
...
fg\_av\_ger\_lr & left-right luminance difference & \avFgavgerlrAll & \avFgavgerlrI & \avFgavgerlrII & \avFgavgerlrIII & \avFgavgerlrIV & \avFgavgerlrV & \avFgavgerlrVI & \avFgavgerlrVII & \avFgavgerlrVIII \tabularnewline
...
fg\_av\_ger\_ml & mean luminance & \avFgavgermlAll & \avFgavgermlI & \avFgavgermlII & \avFgavgermlIII & \avFgavgermlIV & \avFgavgermlV & \avFgavgermlVI & \avFgavgermlVII & \avFgavgermlVIII \tabularnewline
fg\_av\_ger\_pd & perceptual difference & \avFgavgerpdAll & \avFgavgerpdI & \avFgavgerpdII & \avFgavgerpdIII & \avFgavgerpdIV & \avFgavgerpdV & \avFgavgerpdVI & \avFgavgerpdVII & \avFgavgerpdVIII \tabularnewline
...
fg\_av\_ger\_ud & upper-lower luminance difference & \avFgavgerudAll & \avFgavgerudI & \avFgavgerudII & \avFgavgerudIII & \avFgavgerudIV & \avFgavgerudV & \avFgavgerudVI & \avFgavgerudVII & \avFgavgerudVIII \tabularnewline
\end{tabular}
\end{table*}
...
Korrelation zwischen AO und AV?; Also, außer bei fg\_av\_ger\_lr\_diff (wobei da keiner weiss, was das bedeutet)}
...
\begin{figure*}
    ...
    \caption{Pearson
        ...
        \ref{tab:av-events}) rounded to the nearest tenth.
        The correlation
        between the two stimuli's root mean square loudness and between their
        left-right difference in loudness yielded the highest correlation
        values (\texttt{fg\_ad\_rms} and \texttt{fg\_av\_ger\_rms}, r=.7635
