...
\begin{enumerate}
\item Speech is new
\item No frame-wise info, but high-level cinematographic semantics -- however, only for the movie stimulus
\item Conclusion: stimuli are suitable, and on-par for conventional ones
\item Compare the naturalistic results with each other, anterior bias (already hint on audio localizer)
\item issues with visual model, call for more confounds
\item how different from aziz?
\item impact of no task?
\item particular composition of audio contrast vs optimal visual stimuli
\item compare audio results to block-design localizer, again anterior bias, not an averaging effect
\item cite poldrack2007 to state that the procedure in sengupta was better, but we nevertheless did not do it...
\item actual discussion of anterior bias
\item other results RSC LOC
...
% hypo
We hypothesized that due to the complex nature of the naturalistic stimulus,
...
The current model-driven approach required a detailed annotation of the
occurence of relevant stimulus features.
% event counts ftw
The annotation of both types of naturalistic stimuli, a movie and its
audio-description, revealed the respective number of incidentally occurring
...
The presented evidence on the in-principle suitability of a purely auditory
paradigm for localizing the PPA may offer a path to the development of
diagnostic procedures more suitable for individuals with visual impairments or
...
baldassano2013differential, baldassano2016two,
baumann2016functional}.\todo{claim hängt hier etwas in der Luft; wird später
