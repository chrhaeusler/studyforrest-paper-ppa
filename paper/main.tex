...
is a visual area in the human ventral stream with increased hemodynamic
activity correlated with the perception of landscape photos compared to faces or objects.
% AV, AD: operationalization
Here we investigate the perception of
spatial information in 14 individuals from the open-data resource \textit{studyforrest.org}
that were watching a two-hour Hollywood
movie and listened to the movie's audio-description.
% auditory localizer?
%in order to explore if a naturalistic auditory narrative could be used
%to localize a ``visual area''.
% GLM
We model hemodynamic activity based on annotations of selected stimulus features,
% VIS
...
On a group level, a movie stimulus is associated with
increased activation spatially overlapping with a traditionally
localized PPA.
% results: group AD
In contrast, activation for an auditory stimulus is more restricted
to the anterior PPA.
% results: individual AD
On an individual level, hemodynamic responses
form bilateral clusters in the PPA of nine individuals and unilateral
clusters in another two.
% conclusion: generalizability
Results suggest that activation in the PPA generalizes beyond
pictures of landscapes to spatial information embedded in
a naturalistic movie and even an auditory narrative, and may
...
constructed from LEGO blocks \citep{wolbers2011modality}.\todo{cite Lescroart \&Gallant,
Human Scene-Selective Areas Represent 3D Configurations of Surfaces, Neuron, 2019;
semi-naturalistic, but not taskfree, 800+ stimulus, not "a small set", 6 subjects}
...
% AV stimulus
The operationalization of visual spatial perception is based on an annotation of cuts
and depicted locations in the audio-visual movie ``Forrest Gump'', while
% AD stimulus
non-visual spatial perception is investigated using an annotation of
spatial references in the speech occurring in the movie's audio-description.
% GLM contrasts
We applied model-based, mass-univariate analyses to a series of datasets
with BOLD fMRI data from naturalistic and conventional block-design stimulation
from the same set of participants, available from the open-data resource
\href{http://www.studyforrest.org}{studyforrest.org}
\citep{hanke2016simultaneous, hanke2014audiomovie, sengupta2016extension}.
\todo{cite Häusler \& Hanke 2021}
...
can be found in the supplemental material. \todo{check if redundant with later refs}
