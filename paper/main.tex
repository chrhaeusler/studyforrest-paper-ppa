...
\begin{itemize}
    ...
    \item in main text, add references to supplemental material
...
\noindent Discussion
\begin{itemize}
    \item summary \& overall conclusion (last paragraph or independent section)
\end{itemize}
...
%
\todo{imo its better to focus on the cinematographic rationale and drop the last
sentence 'cause activity in e.g. FFA adopts too -> we might substract an on
average smaller response}
...
\todo[inline]{COH: tried to make clear why we did not/could not use ``places vs.
faces''}
...
% (2010)
Notably, localization of the PPA in individual subjects that was done by
...
subjectively good balance between a) the amount of available data, b) how
well the average of events within represented spatial information and
non-spatial information respectively (cf. section \ref{rationale}).
% AV: primary contrast
...
\texttt{vpe\_old} the most frames depicting persons, faces, or objects [(cf.
rationale to categorize the cuts in section \ref{rationale})].
% AD: primary contrast
...
\todo[inline]{in detail: \texttt{fahead}: just N=83; mostly parts
 of face (``ear'', ``nose''), or vague description (``his/her face''; N=10)}
...
soundscape that might already indicate a change of settings.] \todo[inline]{for
sake of conciseness, it is imo better to drop that ``soundscape-thing''}
...
in two subjects (sub-04, sub-20).\todo{I shortened the following part (maybe too
much)}
% Sengupta: 1 of 3
...
\todo{Bartels' clusters are actually pretty big too}
...
\todo[inline]{concluding statements regarding exploited cinematographic
confound? e.g.: movies are nice but have idiosyncrasies? so, maybe you should be
aware of these idiosyncrasies (and control for it) in case you wanna use movies
for research?}

\todo[inline]{overall main message / concluding statement of this
...
different features inherent in visual compared to verbal stimuli.\todo{ouff}
...
\todo[inline]{AD clusters in LOC are more medial in AD stimulus compared to AV;
whereas clusters in PPA and RSC of both stimuli overlap}
% conclusion
...
\todo[inline]{join summary \& conclusion}
...
generating new findings \& hypothesis.\todo{if there are any parts that can be
rephrased as explicit hypothesis: do it!}
...
\todo[inline]{I think it makes sense to report results of AV and discuss AV,
and then report results of AD and discuss AD}
...
\texttt{no\_cut} categorie to a movie cut category (see Table
...
\todo[inline]{changes of a soundscape that indicate a setting's spatial layout
    could correlate with increased activity in the PPA (arnott2008crinkling
    finds increased activations in PPA to sounds made by objects that reveal
    object's material/identity); but: when looking at the data I see no
systematic in the results that let me assume that}
