...
% Vorgabe < 110 chars incl. spaces
%  jetzt 109 Chars
\title{Hemodynamic responses of the Parahippocampal Place Area to spatial cues in a movie and it's audio-description}
% alt:BOLD responses of the Parahippocampal Place Area correlate with spatial cues in a movie and it's audio-only variant
...
% submissions should exemplify reproducible research by clearly describing all steps in the analysis
% providing supporting source code, and explaining how and where others may access all data underlying the analysis.
...
\end{abstract}
% Intro + Discussion = main text ~ 3,000 words.
% include figure providing schematic overview of the study and assay(s) design
% -> ist overkill; Paradigma ist mit "Film ab" wenig kompliziert.
\section{Introduction}
% our system cannot accept BibTeX bibliography files; authors who wish to use BibTeX to prepare their references should therefore copy the reference list from the .bbl file that BibTeX generates and paste it into the main manuscript .tex file (and delete the associated  \textbackslash{}bibliography and \textbackslash{}bibliographystyle commands).

% yeah, whatever. Arbeite erstmal mit der references.bib
...
% Retrosplenial-Cortex kommt in der jetzigen Einleitung nicht vor, jedoch in Ergebnissen von Movie & Audio-Description
% Muss spätestens in der Discussion diskutiert werden
% bin auch am überlegen, ob ich in den Ergebnissen zu den sagitallen slices durch die PPAs noch sagitale Slices durch die RSCs dazunehme??
A classic example of a functional, higher-level visual area is the ``parahippocampal place area'' (PPA) \citep{epstein1998ppa, epstein1999parahippocampal}. The PPA is located in the posterior parahippocampal gyrus at the boundary to the anterior lingual gyrus and medial fusiform gyrus, both being part of the ventral visual stream \citep{mishkin1982contribution, goodale1992separate}. Increased hemodynamic activity in the PPA correlates with the perception of static pictures of landscapes or landmarks compared to pictures of tools or faces.
...
Results generalize at a group average level to a) mental imagination of landscapes \citep{ocraven2000mental} and b) haptic exploration of scenes \citep{wolbers2011modality}, \todo{this needs some more glue. the following is not just about spoken stimuli} whereas results of a study using spoken, place-related sentences were unclear \todo{should be more specific. unclear could mean 'not well described'} \citep{aziz2008modulation}:
a) In a study conducted by \citep{ocraven2000mental} participants viewed alternating blocks of pictures showing famous faces and familiar places during an initial experimental paradigm.
In a subsequent paradigm, participants were instructed to ``form a vivid mental image'' of the previously viewed pictures. The PPA showed increased activation during imagination of places compared to faces but the imagination tasks showed a smaller activation level compared to the perceptual task.
% Wolbers im Folgenden macht noch ne Connectivitäts-Analyse mit dem Occipital-Cortex; da ist dann keine Connectivity im Vergleich zum visuellen Paradigma; deshalb kann das laut ihm kein mental image sein: "the scene-related increase in coupling with the PPA was significantly stronger in multiple clusters in occipital cortex under visual than haptic stimulation (Figure 2B; Table S2)"
b) In a block design study conducted by \citep{wolbers2011modality} the PPA of sighted participants showed increased activation during a delayed matching-to-sample task of haptically explored scenes constructed from Lego bricks compared to haptically explored abstract geometric objects.
...
c) To our knowledge only one study by \citep{aziz2008modulation} correlated hemodynamic activity of the PPA with spoken sentences. Sentences described generic or famous places (e.g. ``The Taj Mahal faces a long thin reflecting pool'') or faces  (e.g. ``Marylin Monroe has a large square jaw''). Participants were instructed to press a button whenever the sentence described an inaccurate or improbable fact. Surprisingly, activation in the left, but not right, PPA was significantly reduced \todo{reduced relative to what condition?} when participants listened to place-related sentences. Moreover, this effect was only observed in sentences involving famous places.
% Frage: Review von Aminoff (2013) kommt nur kurz als "competing hypothesis"; muss man dann in der Discussion diskutieren, was ggf. gar nicht nötwendig ist, weil's darum im Grunde nicht geht
...
% coolness-faktor von natural stimuli ist in der Einleitung beinahe komplett raus
% data-driven analyses vs. theory-driven analyses ist komplett raus
% kann man in discussion bringen, muss man aber nicht ist ja dann offensichtlich
All studies reviewed above used a small set of carefully designed, simplified stimuli to strictly control experimental conditions. They further relied on / implemented explicit judgment-based tasks to keep participants alert and paying attention. Thus, they cannot / could answer the question how the PPA might behave under life-like conditions. Natural stimuli like movies \citep{hasson2008neurocinematics, sonkusare2019naturalistic} or narratives \citep{honey2012notlost, lerner2011topographic, silbert2014coupled} offer an easy to implement, (continuous,) complex, immersive stimulus that better resembles our experience of a continuous dynamic environment.
% hier wird GLM nicht explizit erwähnt; denke, muss nicht so spezifisch sein
In an early study \citep{bartels2004mapping} manually annotated time points that depict faces and human bodies in a 22 minute clip of the movie \textit{Tomorrow Never Dies}. These time points correlated with increased hemodynamic activity in the ``fusiform face area'' \citep{kanwisher1997ffa} and ``extrastriate body area'' \citep{downing2001bodyarea} respectively.
% hier wird ISC als solches nicht erwähnt
\citep{hasson2004intersubject} took an inverse approach using a 30 minute clip of the movie \textit{The Good, the Bad, the Ugly}. fMRI brain volumes showing the highest hemodynamic activity in the PPA [across (five) participants] were matched to the (temporally) correlating movie frames. Results revealed that frames depicted indoor and outdoor scenes including buildings and open fields.
...
% ich habe einiges deiner Kommentare versucht einzubauen
% wenn dir etwas nicht explizit genug ist, fühl dich, frei mir auf die Finger zu hauen oder es vorzugsweise bei wenig Aufwand direkt umzuformulieren;
% bzgl. open source habe ich nichts geschrieben, weil ich nur weiß, das der Code veröffentlicht werden soll. Du hast die Vision diesbezüglich im Kopf und entsprechend auch kanonisch-prägnante Formulierungen und wo die am besten wie hinkommen
% MIH:
% 1. is such stimulus (more) suitable for distinguishing this cognitive process from others?
% 2. given its suitability how do results (PPA-like response localization) differ from a standard visual/block-design localizer for:
% 1) complex visual stimulation without a task, and
% 2) complex auditory stimulation without an explicit imagery task
% both aspects together can then be discussed in terms of the feasibility of (multi-dimensional) localizer development with naturalistic stimuli
% Präteritum oder Präsens? Ist momentan gemischt; find's ok so.
In this study, we analyze data of three publicly available datasets from the same set of 15 participants as part of the studyforrest data set (\href{http://www.studyforrest.org}{studyforrest.org}). We exploratorily investigate how the parrahippocampal gyrus responds to ``spatially relevant'' events embedded in a 2h audio-visual movie \citep{hanke2016simultaneous} and it's audio-description of same length \citep{hanke2014audiomovie}. Results are compared to PPA localizations that were obtained from a dedicated visual localizer experiment \citep{sengupta2016extension}. Both natural stimuli were originally designed to entertain an audience and to capture attention free from any perceptual or behavioral task. We annotated events of the otherwise unknown temporal structure of both stimuli to be able to fit a theory-driven model. In/for the movie, we annotated different kinds of cinematographic cuts \citep{haeusler2016annotation} as presumably ``spatially relevant'' events. Our rationale was that cuts re-orient the viewer in the depicted environment independent from the exact visual content in the movie frame following the cut. In/for the audio-description, we annotated clues about the missing visual content given by the narrator \citep{haeusler2020spechanno}. We correlated regressors drawn from the annotations in order to detect possible/possibly confounded regressors that might prevent functional isolation of the PPA. In conformity to the analyses performed in the studies reviewed above, we submitted our regressors to a standard multi-level, voxel-wise general linear model (GLM). For practical reasons (+++es war a) viel einfacher als visual content zu annotieren und ist b) viel interessanter weil non-visual+++) and encouraged by \citep{aziz2008modulation} and \citep{aminoff2013role}, we focus on the audio-description as a possible life-like substitute to individually localize a ``visual area'' in visually impaired persons.

\todo[inline]{the last paragraph contains a bunch of methods already. I would reduce this to talk about
the general approach: annotation of events, investigation of model quality, cross-analysis of datasets.
Possibly cite https://psyarxiv.com/sdbqv/ and declare that this study is a concrete realization of that
promise}

\section{Methods}
% Ich habe die zwei Methodenteile gemerged und vor allem gekürzt;
% Einige Stellen sind aber sicherlich noch zu lange; denn im Zweifel habe ich es stehen gelassen
% Specific data outputs should be explicitly referenced via data citation (see Data Records and Data Citations, below).
...
\todo{following is a brief summary of most important aspects}

\todo[inline]{we should consider making a statement how often these datasets have been analyzed be
independent groups already. if we do, they will all get an 'you have been cited' alter eventually}

\subsection{Participants}
% ggf. mit "Stimuli" zusammenfassen, weil sich das, dadurch dass die gleichen 15 in beiden Experimenten mitgemacht haben schwer prägnant trennen lässt.
In the first \todo{better to give them labels in the initial paragraph above, we will be referring to them quite a bit} study, fifteen participants (all right-handed, age 21–38 years, mean age 26.6 years, 12 male) listened to the German audio-description (Koop, Michalski, Beckmann, Meinhardt \& Benecke, produced by Bayrischer Rundfunk, 2009) of the movie ``Forrest Gump'' (R. Zemeckis, Paramount Pictures, 1994) as an additional audio track for visually impaired listeners on Swiss public television. In the second study, the same 15 participants (21–39 years, mean age 29.4, six female) watched the audio-visual movie with dubbed German audio track. All participants reported to have normal hearing, normal or corrected-to-normal vision, and no known history of neurological disorders. In both studies, participants received monetary compensation and gave written informed consent for their participation and for public sharing of all obtained data in anonymized form. The studies had prior approval by the Ethics Committee of Otto-von-Guericke University of Magdeburg, Germany.

\todo[inline]{i think the stimulus infos need to move from here into the section below}

\subsection{Stimuli}
% grey bars habe ich nicht erwähnt, obwohl sie - weil bei Lisas Stimuli überall anders - so viel Spaß gemacht haben
...
\todo{possibly cut the rest of the section} At the start recording session(s), participants listened to music from the movie’s closing credits segment to individually determine optimal stimulus volume. Participants were instructed to ``maximize the volume of the stimulus without it becoming unpleasantly loud or causing acoustic distortions by overdriving the loudspeaker hardware''.
% unterscheidet sich ggf. zwischen Hörspiel / Film oder kann ganz raus?
...
% Einige Dinge können m.M.n. raus; insb. das in den []-Klammern
% habe es noch einmal drinnen gelassen, solltest Du die Dinge für wichtig halten
...
% Hier hat es Unterschiede zwischen Hörspiel & Film geben? habe es wie folgt zusammengefasst; im Zweifel ist es eh noch zu ausführlich
...
% einleitendes Blabla zu haeusler2020speechanno
% deutlich machen, warum was wie annotiert wurde
...
% aus Speech-Anno Paper 'descriptive-statistics-anno.py' übernehmen & anpassen.
% von allen \newcommands wurde das \ for dem Variablennamen entfernt
% -> sind insofern alles Platzhalter bis das Skript fertig ist, das die Inputdatei schreibt.
\begin{table*}[t]
...
% \todo{shouldn't this be also just in Methods?}
% hier ggf. noch expliziter machen, dass die spatially downsampled sind
...
\todo{cite sengupta2016 where this was first done and described, ie same image/data was used here}
...
\item   https://en.wikipedia.org/wiki/Bland%E2%80%93Altman_plot
\end{itemize}
