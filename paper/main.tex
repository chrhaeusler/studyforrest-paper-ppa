...
\begin{itemize}
\item Struktur des letzten Absatzes finde ich
    bisher nicht so geil (Hypothesen kommen recht spät (d.h. nach Methode),
    Results folgen direkt darauf
\end{itemize}
...
\item auch VIS hat mehrere Kontraste
\item neuer Abschnitt 2.6: all regressoren und Kontraste; Kontraste sind abiträr
    aber ist ok; 2.7: eigentliche Statistische Analyse
...
\todo{phrasing might be strange but aims to delineate
from Huth (2016); they do voxel-wise semantic modeling regarding places in PPA
but imo that does not say anything about excitation/inhibition}
% Aziz' stimuli
...
contrasting selected stimulus features that incidentially occur under more
...
hemodynamic activity in the PPA as functionally defined by a visual localizer
experiment using static pictures done with the same subjects
...
hemodynamic activity in [roughly] the same brain regions as a block-designed
visual localizer experiment.\todo{interpretation?}
...
%
Lasty, our results suggest that the PPA's anterior part can be localized in
individual subjects by contrasting high-level semantic information occurring in
an auditory naturalistic stimulus.

\todo[inline]{the topic anteriorer vs. posterior is mentioned first at the very
and of the intro and occurrs as "surprising" in the intro as in our results
(it's mentioned in the abstract though)}
...
\unit[23.75 $\times$ 10.25]{cm}.\todo{replace with visual angle size, in line with VIS}
% angle of view: VIS
