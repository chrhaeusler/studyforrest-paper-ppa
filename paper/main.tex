...
and faces, or objects.\todo{judgment task not mentioned anymore}
% Aziz' tasks: Participants were instructed to press a button whenever the
% sentence described an inaccurate or improbable fact. Aziz' results:
...
\citep{epstein1999parahippocampal, grill2006repetition}.\todo[inline]{doesn't
make sense not 'cause this should be true to the face of forrest too}
...
\centering
    \includegraphics[width=\linewidth]{figures/regressor-corr} \caption{Pearson
        ...
        \todo[inline]{not all labels are actually listed in that table; which
        ones are not listed? checked it, all labels listed}
      }
\label{fig:reg-corr}
\end{figure*}
...
objects.\todo{maybe speak of ratio}
% AD: primary contrast
...
\texttt{furniture} to get a sufficient amount of events per stimulus
segment.
% furniture
...
\todo[inline]{paragraph above could state that ``fahead'', just N=83; mostly
    just parts (``ear'', ``nose'') of face, or vague description ``his/her
face''}.
% control contrasts
...
% AD is the real shit
Furtherm, we hypothesized that an exclusively auditory
stimulus could localize the PPA in individual subjects and thus
possibly substitute a classic visual localizer experiment.

\todo[inline]{part regarding Sengupta is shortened; they used $t$-maps???}
% Sengupta et al., 2016
...
% Sengupta: 1 of 3
Individual PPAs were assessed by titrating [?] the $t$-threshold in the range
of [2, 3] in the thresholded $t$-maps (Z>1.64, p<.05 cluster corrected) of three
different $t$-contrasts (\textit{strict}, \textit{relaxed}, and \textit{simple}
stimulus set cf.  \citep{sengupta2016extension}) until we found an isolated
...
occurring in an audio-visual movie and its audio-description.
% group AV
...
% group AD
Results for the audio-description reveal significant activation in the PPA but
restricted its anterior part.
% individual level
On an individual level, results of the audio-description show significant
bilateral clusters in the anterior PPA of nine subjects and significant
...
%
Contrary to \citep{bartels2004mapping} who annotated the movie frames' content
(color, faces, language and human bodies), we categorized each frame after a cut
independent of its content. \todo{PPA in \citep{hasson2004intersubject} but do
reverse ISC; Bartels' clusters are actually pretty big too}
% hypo
Despite this ad hoc approach, we hypothesized that [confounding variables
...
to the averaging process that did not control for confounding visual features or
the attentional focus of study participants.

\todo[inline]{I am pretty surprised the clusters are not all over the brain.  I
do not know where/how do phrase it but should be mentioned imo}
...
\todo[inline]{nice that the following is mentioned but what to do with it?
Movies are nice but have idiosyncrasies; so, maybe you should be aware of these
idiosyncrasies (and control for it) in case you wanna use movies for research?}
...
and to later focus more on persons and objects relevant to the evolved plot
later  \citep{brown2012cinematography, mercado2011filmmakers}.

% summary: nevertheless
\todo[inline]{Overall summary for AV: so, yes, what is the main message of this
...
places vs famous faces; I think they ran their f-test on the average of PPA
voxels defined by separate localizer}
...
that were averaged across voxels of ROIs (PPA and fusiform face
...
compared to activity in the FFA, and only for sentences describing famous places
compared to famous faces.
% we: model & whole brain
Here we modeled events from on- to offset of single words and performed an
...
\todo[inline]{at the moment positioned here; makes sense at a couple of other
positions; not sure yet}

% optimal stimulus type in vision
In the visual domain, landscapes and not pictures of landmarks or buildings are
considered to be the ``optimal'' stimulus type
\citep{epstein2008parahippocampal}.
% we did not choose se\_new and se\_old
We did not include the categories that contained switches from one setting to
another (\texttt{se\_new} and \texttt{se\_old}) which one might assume to
contain the auditory equivalent to the ``optimal'' visual stimulus type in the
primary AD contrast.
% why
...
of localizer}
%
Based on the fact that the annotation of nouns offered a more precise way to
operationalize the perception of spatial information than the annotation of cuts
and locations, we also hypothesized that analyzing the data from the
audio-description could localize the PPA in individual persons.
%
Hence to test if an auditory narrative could [in principle] substitute a visual
localizer experiment, we examined the results from the audio-description's
primary contrast on an individual level.
...
judgment, we used a ``one threshold fits all approach`` (Z>3.4, p<.05
cluster-corrected) in one contrast.
...
reveal that clusters are spatially restricted to the anterior part of individual
PPAs.
% conclusion
Thus, the increased hemodynamic activity in the anterior part of the PPA
revealed on the group level is not just a by-product of the averaging process
[as it can reliably be found in individual study participants.]
% conclusion
Lastly, our exploratory analysis suggests that a naturalistic auditory stimulus
...
\todo[inline]{probably not important but: what do results suggest
regarding individual differences?  Differences in alertness, attention to
spatial information, predisposition?  ``capability'' to process auditory spatial
information?}

% question
This raises the question if the different activation patterns can be attributed
to different features inherent in visual compared to verbal stimuli.
...
\subsubsection{not many random cluster + naturalistic stimuli are cool}
...
\subsubsection{naturalistic stimuli + data re-use, fuck yeah}
...
% anterior vs posterior pattern
In contrasts 1 to 6 of the AD stimulus, we find consistently significant
clusters spatially constrained to the anterior part of the PPA group overlap.



\subsubsection{RSC \& LOC}

\todo[inline]{this part is now super short}
...
\todo[inline]{hm, so why did I build contrasts of the no\_cut condition compared
...
\todo[inline]{shower thought: changes of a soundscape that indicate a setting's
    spatial layout could correlate with increased activity in the PPA
    (arnott2008crinkling finds increased activations in PPA to sounds made by
    objects (that reveil object's material/identity); but at the moment when
    looking at the data I see no systematic in the results that let me assume
that}
