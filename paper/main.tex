...
setting.
%
\todo[inline]{it's probally not about the soundscape anymore (to speculative)
but about the somewhat correlation of nouns and cuts}
...
In line with previous studies, we also operationalized spatial perception via
general linear model (GLM) $t$-contrasts.
...
restricted to its anterior part.
% individual level
On an individual level, results of the audio-description show significant
bilateral clusters in the anterior PPA of nine subjects and significant
...
independent of its content. \todo{Bartels' clusters are actually pretty big too}
% hypo
...
% solution
Future studies that aim to use a movie to localize visual areas should
...
\citep{brown2012cinematography, mercado2011filmmakers}.
%
\todo[inline]{nice that exploited confound structure is mentioned but what to do
with it? hey, look, it works? movies are nice but have idiosyncrasies? so, maybe
you should be aware of these idiosyncrasies (and control for it) in case you
wanna use movies for research?}

% summary: nevertheless
\todo[inline]{Overall summary for AV \& concluding statement!?!}

\todo[inline]{so, yes, what is the main message of this
``cross-modal-control-movie-thing''??}
...
individual PPAs.
% conclusion
Thus, the cluster location and size is not just a by-product of the averaging
process but the same pattern can can reliably be found in individual study
participants.
% question
...
% RSC intro
Like the PPA, the RSC and LOC are known to be scene-selective regions (see
...
\todo[inline]{same part as in the main text but with number of contrasts}
