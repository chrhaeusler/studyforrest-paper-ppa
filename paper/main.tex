...
\todo[inline]{SE: habe mich mit 2.8 extrem schwer getan: Länge des Abschnitts,
oft komplizierten Beschreibungen}
...
\todo[inline]{SE: Table 2 ist relativ kryptisch bzw für jemanden, der sich zum
    ...
    redundant since the counts are not the count of events of the actual
...
\todo[inline]{SE: das mit den "five additional categories" habe ich nicht
verstanden. Überhaupt finde ich die ganze Beschreibung relativ schwer zu
durchdrungen, und ich glaube damit werde ich nicht alleine sein. COH: }
...
\todo[inline]{"Continuous bins lasting 40ms" ?!?; COH: goddamnit! I've rewritten
    ...
    \citep{wilson2008beyond} who did the same}
%
...
created as a control condition and to create negative control contrasts:
% procedure
...
\todo[inline]{mention low-level feature events for AV here as well}
...
% why several contrasts
To test the robustness of our approach,  we created several contrasts for every
stimulus.
%
We operationalized the perception of spatial information compared to the
perception of ``non-spatial'' information differently in eight $t$-contrasts for
the AO stimulus (see Table \ref{tab:ao-contrasts}) and five $t$-contrasts for
the AV stimulus (see Table \ref{tab:av-contrasts}).
% reason: robustness
We assumed all contrasts could in principle correlate with increased hemodynamic
activity in the PPA.
% but we expect differences because
Nevertheless, we also assumed contrasts would differ in \todo{differ in what?
``localization performance'' isn't well phrased. How ``well'', ``nice'' the
results are?} since used event categories differed a) in how well the averaged
events reflected spatial information, and b) in event counts
% thus: primary contrasts
We chose one ``primary contrast'' for every stimulus before running the
analyses.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In the primary contrast of the AO stimulus we contrasted \texttt{geo} and
\texttt{groom} with all non-geometry related categories
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In the primary contrast of the AV stimulus we contrasted
...
% for example, AO stimulus AO COPEs: intro
For example, the categories \texttt{se\_new} and \texttt{se\_old} of the AO
stimulus were not included in the primary contrast because
...
interest in any contrast.
%
In a serial stream of auditory information, these nouns could be perceived
differently by different subjects as either a geometry-defining scene elements
or as an isolated object.

% why not locale change
