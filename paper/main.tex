...
\begin{itemize}
    ...
    \item adapt captions of result tables to new phrasing in intro / method
...
% study goal
The goal of this study was to investigate if annotated cues about the spatial
layout embedded in two naturalistic stimuli correlate with increased hemodynamic
activity in the PPA that was defined using a visual localizer experiment within
the same subjects \citep{sengupta2016extension}.
% group level
...
corrected) to results gained from the visual localizer.
% individual level
...
substitute for a visual paradigm to localize the PPA.\todo{``localize''}
% NeuroVault
Unthresholded $z$-maps of all contrasts on a group level as well as individual
results of the primary $t$-contrasts co-registered to the group-template can
found at \href{https://neurovault.org/collections/KADGMGVZ/}{NeuroVault}.
\todo{still private collection}
...
% AV intro
First, we analyzed data from the audio-visual movie offering a visual but
...
\paragraph{individual results}

\todo[inline]{if we here report procedure of Sengupta as detailed as it is now
(imo too long), we probably need to explain the differences between strict,
relaxed, simple in more detail in the method section and not just reference to
Sengupta}
...
of the averaging process.\todo{if you know what I mean}
% AD is the real shit
...
space (Z>2.3, p<.05 cluster-corrected) and group space ).
...
corrected) can be found in the accompanying datset \todo{Neurvault does not like
unstandardized spaces}
