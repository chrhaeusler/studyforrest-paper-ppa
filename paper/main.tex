...
% present compelling new findings & conclusions derived from published.  PPA
...
sounds \citep{van2017development}.\todo{reading the paper: activation is
non-significant, lol}
% van der Hurk: 64 sounds x 1.8 seconds; 20 sighted, 14 congenitally blind
\todo{cite Lescroart (2019). semi-naturalistic, but task, 800+ stimulus, N=6}
...
Participants performed a one-back image matching task for four block-design runs,
with two \unit[16]{s} blocks per stimulus category in each run.
%
For details on stimulus creation and presentation see
...
% AD
In the audio-description study, visual instructions were presented on a rear-projection screen
...
fatigue.
% AV & VIS
In the movie and block-design localizer study, visual instructions and stimuli were presented on a
rear-projection screen
% screen size \unit[23.75 $\times$ 10.25]{cm}
at a viewing distance of \unit[63]{cm}, with a movie frame projection size
of approximately \unit[21.3]$^{\circ}$ $\times$ \unit[9.3]$^{\circ}$.
% angle of view: VIS
In the block-design localizer study, stimulus images were displayed at a size of approximately
\unit[10]$^{\circ}$ $\times$ \unit[10]$^{\circ}$ of visual angle.
% AD & AV: auditory stimulation
Auditory stimulation was implemented using custom in-ear (audio-description), or over-the-ear
headphones (movie), which reduced the scanner noise by
at least \unit[20–30]{dB}.


\subsection{fMRI data acquisition}

Gradient-echo fMRI data for the audio-description study were acquired using a \unit[7]{Tesla}
...
\begin{table*}[tbp]
    ...
    % negative control Tab.~\ref{tab:ao-events}.
    }
\label{tab:av-events}
\small
...
% what it depicts
Each subplot visualizes the mean value and difference (localizer minus audio-description) of all voxels in temporal and occipital cortices. The marginal distributions
of the mean scores indicate a general agreement of both contrast scores across
voxels in the PPA localization overlap as a probabilistic indicator (blue), and the
individual block-design localizer results (red). Notably, 11 participants exhibit
a pattern of increased $Z$-scores for the naturalistic stimulus (lower right
quadrant) that includes voxels labeled by the block-design localizer, but
also additional voxels.
...
auditory semantic stimulation are less clear \citep{aziz2008modulation}.
...
specifically and predominantly evoke the processing spatial information, we
employed a naturalistic movie stimulus that was designed for entertainment.

% PPA: intro
In an earlier study, \cite{bartels2004mapping} manually annotated the content
of individual movie frames (color, faces, language, and human bodies) and found
that the expected specialization of functional areas is preserved during movie
watching.
%
In contrast, we aimed to exploit a cinematographic confound in the stimulus
structure, where
...
\citep{brown2012cinematography,
katz1991film, mascelli1998five}.
% hypo
We\todo{following two sentences are confusing imo} hypothesized that due to the complex nature of a naturalistic stimulus,
unrelated factors would be balanced across a large number of events, and make
the bias of spatial information availability in particular events accessible to
convential model-based statistical analysis of BOLD fMRI data.
%
This approach required a detailed description of the occurence of relevant
stimulus features. For both types of naturalistic stimuli used here, a movie
and its audio-description, this annotation revealed the respective number of
incidentally occurring events to be similar to those of a conventional
experimental paradigm used in neuroimaging, for the purpose of localizing
particular regions of interest.

% group AV
On a group-average level, results for the audio-visual movie show significantly
...
% individual level
Moroever, the group averages are representative for the majority of individual
results, where, for example, the audio-description stimulus yields bilateral
clusters in the anterior PPA of nine participants and a unilateral cluster in
one participant.
...
The presented evidence on the in-principle suitability of an auditory-only
paradigm for localizing the PPA may offer a path to the development of
diagnostic procedures more suitable for populations of individuals with visual
impairments or conditions like nystagmus.
% meaning: aPPA vs. pPPA
Our results provide further evidence that the PPA can be divided into functional
subregions \citep{aminoff2007parahippocampal, aminoff2015associative,
baldassano2013differential, baldassano2016two, baumann2016functional}.

% results
Group results of the movie stimulus' primary contrast yielded a large cluster
that spans the group overlap of individual PPA ROIs from anterior to posterior.
% confound of higher perception
The cluster's extension into more posterior, earlier visual areas could be
attributed to the temporal averaging across events that did not control for
confounding visual features.
% solution
Future studies that aim to use a movie to localize visual areas in individual
participants should extensively annotate the content of frames \citep[e.g., using the
open-source solution ``Pliers'' for feature extraction from a visual
...
% hypothesis
Based on the report by \cite{aziz2008modulation}, we hypothesized that semantic spatial
information embedded in the audio-description would correlate with
...
In the current study, we modeled events from onset to offset of single words and performed a
...
Contrary to \cite{aziz2008modulation}, our group results suggest that auditory
...
This naturalistic paradigm nevertheless differs from \cite{epstein1998ppa},
...
speculated on the purpose of the investigation or perform undesired and unknown
evaluation or categorization of isolated stimuli.
...
%
Whereas \cite{sengupta2016extension} chose the subjectively ``best fitting''
$t$-threshold in a $t$-map from one of three contrasts by visual judgment, we used
...
$Z$-scores in the anterior part of PPA are higher in audio-description contrasts than in contrast
of localizer}
...
inherent in the visual stimuli compared to the purely auditory stimulus.
...
abstract category \citep{marchette2015outside, watson2016patterns}) and
objects (e.g. spatial contextual associations \citep{aminoff2007parahippocampal, aminoff2013role}).
% connectivity
...
(e.g. spatial contextual information \citep{aminoff2007parahippocampal}) as immanent in both visual and
verbal stimuli.
...
\href{https://openneuro.org/datasets/ds000113}{ds000113} (2014). \todo{4
datasets \& using authors/year of the audio-description}
...
\todo[inline]{random [?] prefrontal cluster in 2-3 AD contrasts; at the moment
no idea what the correlating process could be; be aware of reverse inferences}


\subsection{Control contrasts}

\todo[inline]{discuss results in light of ``negative control'' (-> definition \&
purpose)}

% ---------- AV control contrasts ----------
% 6 nothing 7 nothing 8 nothing 9 nothing
%10 se_new > se_old (nouns but in AV stimulus): LOC (l/r) right PPA, bilateral
%sup. lat. occ. c., sup. parietal lobe (right)
