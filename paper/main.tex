...
\todo[inline]{but: Aminoff (2013) also reviews episodic memory function \& tries
to unify domains via contextual information hypothesis which is not mentioned at
all here}

\todo[inline]{Lescroart \& Gallant (2019). semi-naturalistic, but task, 800+ stimulus, N=6}
...
preceding paragraph; COH: but imo the last two paragraphs are about exactly
that??}
...
\todo[inline]{other studies that localize PPA: essentially every study that does
something "PPA-related" does it and does it using pictures; there are two
studies that evaluate the localization as such and both give "pretty" results:}

\todo[inline]{onlinelibrary.wiley.com/doi/full/10.1002/hbm.20116 -> 6 subjects;
6x bilateral clusters}

\todo[inline]{academic.oup.com/cercor/article/16/10/1453/353981 -> 12 subjs: 12x
bilateral clusters}
...
\todo[inline]{SE: temporal order of the (naturalistic) stimuli is not unknown,
you know it by looking at the movie; COH: yes, but actually no.}
...
naturalistic stimuli have fixed but initially unknown temporal structure
...
in the AV-movie are relevant in the current context; COH: imo that is more
important in context of the contrasts (-> check if it is sufficiently described
there); might fit in here, too, but makes current part unnessecarily
complicated}
...
\todo[inline]{add a summary to the end of section 2.7 explaining in 2 sentences,
what regressors you will be focussing on -> easier for those that got confused
on the way}
% AV design matrix
The design matrices of the first-level time-series GLM analysis comprised the
regressors for the stimulus features listed in Tables~\ref{tab:events} and
\ref{tab:events} for the movie and audio-description stimulus, respectively.
% AD regressors in AV
...
listed Table~\ref{tab:events} (excluding \texttt{no\_cut}).
...
\texttt{vpe\_old} in segment 3; Tab.~\ref{tab:events}).
...
\todo[inline]{"we, subjectively, picked a primary contrast" :-(  -> you actually
write "based on"}
%
...
listed in Table~\ref{tab:contrasts} for movie and audio-description,
respectively. For each stimulus we, subjectively, picked a primary contrast for
result presentation (the first one in each Table) based on how well the average
event within categories represents spatial and non-spatial information, and the
number of events in the stimulus.  An evaluation of the robustness of these
results with respect to similar but different contrasts is provided in the
supplementary materials.

\todo[inline]{"vse, vno, vpe,..." are not really the most intuitive regressor
names -> ausschreiben, wo möglich}
% AV: primary contrast
...
\todo[inline]{The presentation could be structured a bit more (subheadings,
paragraphs etc), as it is, it is quite a long flow of text and numbers -> check
instructions at scientific data}
...
Bland-Altman-Plots -> mention that point at BA-Plots}
...
\todo[inline]{"a visual but ecologically more valid stimulation" -> The "but"
doesn't make sense to me here -> add phrasing similar to the one used in the
(rephrased) intro}
% AV intro
...
PPA? COH: part of the discussion; maybe a concluding statement here}
...
\todo[inline]{jeweils mit dem Befund, den Du diskutieren willst beginnen und mit
einer take-home message enden.}

\todo[inline]{Was mich etwas wundert, es gibt
doch schon andere "event-related" analysen von movies, oder? Die werden kaum
diskutiert. -> Studie, die etwas mit Event-related design zitieren;
NeuroScout-Framework oder. Pliers Studien zitieren}

\todo[inline]{Ebenso kommt mir das Potential (warum ist das relevant -> rich
stimulus ie many possible analyses, better compliance, easier to perform for
difficult populations, etc) eigentlich zu kurz}
...
\todo[inline]{es wird jetzt lediglich in den results erwähnt, dass Sengupta
einen von dreien benutzt hat}
%#11 question
...
\todo[inline]{print contrasts; count per category? -> pattern "average of
events" and counts vs. significant clusters?}
...
% intro
For each contrast we created several control contrasts.
% AV: no_cut > x
...
shots (\texttt{no\_cut} category) with movie cut related categories (Tab.~\ref{tab:contrasts}).
%
None of these four contrasts yielded a significant cluster which suggest that
XYZ\todo{concluding statement}.
...
% \texttt{vse\_old}.
Given that cuts to a setting depicted for the first time (\texttt{vse\_new}) and
nouns indicating a a switch to a setting ocurring for the first timei
(\texttt{se\_new}), and cuts to a recurringly depicted setting
({\texttt{vse\_old}) and nouns indicating a switch a recurring setting
(\texttt{se\_old} turned out to be moderately correlated
(Fig.~\ref{fig:reg-corr}), we created a contrast that used the
audio-description's nouns on the movie data ({\texttt{se\_new} >
\texttt{se\_old}).
% results
The contrasts yielded a significant cluster in the right PPA and bilateral
clusters in the superior lateral occipital cortices (the right hemispheric
cluster extending into the superior parietal cortex).
%
Results suggest that XYZ.
\todo{concluding statement}
...
order to test XYZ.
\todo{why exactly did we do this except "we do not expect (lots of) significant cluster because negative control?}
%
Two of five contrasts yielded signficant clusters.
% contrast 9
