...
\todo[inline]{\textbf{My notes:}\par
    ...
    2. cross-referencing to Supplementary Figures does not work but show
``??''. Referencing from suppl.tex to main.tex works tough
}
...
% open question
\todo[inline]{I revised the following paragraph; I guess it's okay that way}
...
\todo[inline]{Since we did no analyses in surface space imo it makes more sense
    to explain the ``method'' to create the surface plots here in the
``Results''}

\todo[inline]{only Captions mention smoothing of union of PPAs; Volume FWHM was
2mm, Surface FWHM was 3mm}

\todo[inline]{Merge plots and adjust references here}
...
\centering
    ...
    \caption{Mixed-effects group-level (N=14) clusters ($Z$>3.4, $p$<.05
      cluster-corrected) of activity correlated with the processing of spatial
      information.
      a) Results are displayed on top of the MNI152 T1-weighted head template,
      with the acquisition field-of-view for the audio-description study
      highlighted.
      The results of the audio-description's primary $t$-contrast (blue) that
      compares geometry-related nouns spoken by the narrator to non-spatial
      ...
      individual PPA localization reported by \citet{sengupta2016extension}
      that was spatially smoothed applying a Gaussian kernel with full
      width at half maximum (FWHM) of \unit[2.0]{mm}
      % surface map
      b) The results are displayed as projections onto the reconstructed
      surface of the MNI152 T1-weighted brain template. After projection the
      union of individual PPA localizations was spatially smoothed by a
      Gaussian kernel with FWHM of \unit[3.0]{mm}.
    }
...
\todo[inline]{Maybe add sentence here that ``further studies are obvious''}
