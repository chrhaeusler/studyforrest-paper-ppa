...
\begin{abstract}
% Analysis Paper abstract: < 170 words
% no references are allowed: 
% new analysis of existing data of interest to a broad section of our audience
% highlighting innovative examples of data reuse
% may be used to present compelling new findings and conclusions derived from published data.
% submissions using data previously published at Scientific Data are encouraged
...
% Bzgl. Zitationen (lasse alles so lange wie möglich in Kbibtext's .bib-Datei): \cite{cite1, cite2}
...
Results generalize at a group average level to a) mental imagination of landscapes {ocraven2000mental} and b) haptic exploration of scenes {wolbers2011modality}, \todo{this needs some more glue. the following is not just about spoken stimuli} whereas results of a study using spoken, place-related sentences were unclear \todo{should be more specific. unclear could mean 'not well described'} {aziz2008modulation}:
...
c) To our knowledge only one study by {aziz2008modulation} correlated hemodynamic activity of the PPA with spoken sentences. Sentences described generic or famous places (e.g. “The Taj Mahal faces a long thin reflecting pool”) or faces  (e.g. “Marylin Monroe has a large square jaw"). Participants were instructed to press a button whenever the sentence described an inaccurate or improbable fact. Surprisingly, activation in the left, but not right, PPA was significantly reduced \todo{reduced relative to what condition?} when participants listened to place-related sentences. Moreover, this effect was only observed in sentences involving famous places. 
...
% wurde unter "Regressoren" zu Beginn der 1st-lvl-Analyse verschoben
...
\begin{figure}[htbp]
  \centering
%  \includesvg{regressors-corr.pdf}
  \includegraphics[width=1.0\textwidth]{regressors-corr.pdf}
  % MIH ignore such beautification issues, they are easy to fix at the very end with a manual intervention. Shifting the colorbar by hand to make it look nicer is no threat to reproducibility ;-)
